var movies = {
  "One": {
    "title": "The Shawshank Redemption",
    "genres": "Crime,Drama",
    "directors": "Frank Darabont",
    "cast": " Tim Robbins as Andy Dufresne, Morgan Freeman as Ellis Boyd 'Red' Redding, Bob Gunton as Warden Norton, William Sadler as Heywood, Clancy Brown as Captain Hadley ",
    "rating": "9.3",
    "description": "Two imprisoned men bond over a number of years, finding solace and eventual redemption through acts of common decency."
  },
  "Two": {
    "title": "The Godfather",
    "genres": "Crime,Drama",
    "directors": "Francis Ford Coppola",
    "cast": " Marlon Brando as Don Vito Corleone, Al Pacino as Michael Corleone, James Caan as Sonny Corleone, Richard S. Castellano as Clemenza, Robert Duvall as Tom Hagen ",
    "rating": "9.2",
    "description": "The aging patriarch of an organized crime dynasty transfers control of his clandestine empire to his reluctant son."
  },
  "Three": {
    "title": "The Godfather: Part II",
    "genres": "Crime,Drama",
    "directors": "Francis Ford Coppola",
    "cast": " Al Pacino as Michael, Robert Duvall as Tom Hagen, Diane Keaton as Kay, Robert De Niro as Vito Corleone, John Cazale as Fredo Corleone ",
    "rating": "9.0",
    "description": "The early life and career of Vito Corleone in 1920s New York is portrayed while his son, Michael, expands and tightens his grip on the family crime syndicate."
  },
  "Four": {
    "title": "The Dark Knight",
    "genres": "Action,Crime,Drama,Thriller",
    "directors": "Christopher Nolan",
    "cast": " Christian Bale as Bruce Wayne, Heath Ledger as Joker, Aaron Eckhart as Harvey Dent, Michael Caine as Alfred, Maggie Gyllenhaal as Rachel ",
    "rating": "9.0",
    "description": "When the menace known as the Joker emerges from his mysterious past, he wreaks havoc and chaos on the people of Gotham, the Dark Knight must accept one of the greatest psychological and physical tests of his ability to fight injustice."
  },
  "Five": {
    "title": "12 Angry Men",
    "genres": "Crime,Drama",
    "directors": "Sidney Lumet",
    "cast": " Martin Balsam as Juror 1, John Fiedler as Juror 2, Lee J. Cobb as Juror 3, E.G. Marshall as Juror 4, Jack Klugman as Juror 5 ",
    "rating": "8.9",
    "description": "A jury holdout attempts to prevent a miscarriage of justice by forcing his colleagues to reconsider the evidence."
  },
  "Six": {
    "title": "Schindler's List",
    "genres": "Biography,Drama,History",
    "directors": "Steven Spielberg",
    "cast": " Liam Neeson as Oskar Schindler, Ben Kingsley as Itzhak Stern, Ralph Fiennes as Amon Goeth, Caroline Goodall as Emilie Schindler, Jonathan Sagall as Poldek Pfefferberg ",
    "rating": "8.9",
    "description": "In German-occupied Poland during World War II, Oskar Schindler gradually becomes concerned for his Jewish workforce after witnessing their persecution by the Nazi Germans."
  },
  "Seven": {
    "title": "Pulp Fiction",
    "genres": "Crime,Drama",
    "directors": "Quentin Tarantino",
    "cast": " Tim Roth as Pumpkin, Amanda Plummer as Honey Bunny, Laura Lovelace as Waitress, John Travolta as Vincent Vega, Samuel L. Jackson as Jules Winnfield ",
    "rating": "8.9",
    "description": "The lives of two mob hit men, a boxer, a gangster's wife, and a pair of diner bandits intertwine in four tales of violence and redemption."
  },
  "Eight": {
    "title": "The Lord of the Rings: The Return of the King",
    "genres": "Adventure,Drama,Fantasy",
    "directors": "Peter Jackson",
    "cast": " Noel Appleby as Everard Proudfoot, Ali Astin as Elanor Gamgee, Sean Astin as Sam, David Aston as Gondorian Soldier 3, John Bach as Madril ",
    "rating": "8.9",
    "description": "Gandalf and Aragorn lead the World of Men against Sauron's army to draw his gaze from Frodo and Sam as they approach Mount Doom with the One Ring."
  },
  "Nine": {
    "title": "Il buono, il brutto, il cattivo",
    "genres": "Western",
    "directors": "Sergio Leone",
    "cast": " Eli Wallach as Tuco, Clint Eastwood as Blondie, Lee Van Cleef as Sentenza / Angel Eyes, Aldo Giuffrè as Alcoholic Union Captain, Luigi Pistilli as Father Pablo Ramirez ",
    "rating": "8.9",
    "description": "A bounty hunting scam joins two men in an uneasy alliance against a third in a race to find a fortune in gold buried in a remote cemetery."
  },
  "Ten": {
    "title": "Fight Club",
    "genres": "Drama",
    "directors": "David Fincher",
    "cast": " Edward Norton as The Narrator, Brad Pitt as Tyler Durden, Meat Loaf as Robert 'Bob' Paulsen, Zach Grenier as Richard Chesler, Richmond Arquette as Intern ",
    "rating": "8.8",
    "description": "An insomniac office worker, looking for a way to change his life, crosses paths with a devil-may-care soap maker, forming an underground fight club that evolves into something much, much more."
  },
  "Eleven": {
    "title": "The Lord of the Rings: The Fellowship of the Ring",
    "genres": "Adventure,Drama,Fantasy",
    "directors": "Peter Jackson",
    "cast": " Alan Howard as Voice of the Ring, Noel Appleby as Everard Proudfoot, Sean Astin as Sam, Sala Baker as Sauron, Sean Bean as Boromir ",
    "rating": "8.8",
    "description": "A meek Hobbit from the Shire and eight companions set out on a journey to destroy the powerful One Ring and save Middle Earth from the Dark Lord Sauron."
  },
  "Twelve": {
    "title": "Forrest Gump",
    "genres": "Comedy,Drama,Romance",
    "directors": "Robert Zemeckis",
    "cast": " Tom Hanks as Forrest Gump, Rebecca Williams as Nurse at Park Bench, Sally Field as Mrs. Gump, Michael Conner Humphreys as Young Forrest, Harold G. Herthum as Doctor ",
    "rating": "8.8",
    "description": "While not intelligent, Forrest Gump has accidentally been present at many historic moments, but his true love, Jenny Curran, eludes him."
  },
  "Thirteen": {
    "title": "Star Wars: Episode V - The Empire Strikes Back",
    "genres": "Action,Adventure,Fantasy,Sci-Fi",
    "directors": "Irvin Kershner",
    "cast": " Mark Hamill as Luke Skywalker, Harrison Ford as Han Solo, Carrie Fisher as Princess Leia, Billy Dee Williams as Lando Calrissian, Anthony Daniels as C-3PO ",
    "rating": "8.8",
    "description": "After the rebels are overpowered by the Empire on their newly established base, Luke Skywalker begins Jedi training with Master Yoda. His friends accept shelter from a questionable ally as Darth Vader hunts them in a plan to capture Luke."
  },
  "Fourteen": {
    "title": "Inception",
    "genres": "Action,Adventure,Sci-Fi,Thriller",
    "directors": "Christopher Nolan",
    "cast": " Leonardo DiCaprio as Cobb, Joseph Gordon-Levitt as Arthur, Ellen Page as Ariadne, Tom Hardy as Eames, Ken Watanabe as Saito ",
    "rating": "8.8",
    "description": "A thief, who steals corporate secrets through use of dream-sharing technology, is given the inverse task of planting an idea into the mind of a CEO."
  },
  "Fifteen": {
    "title": "The Lord of the Rings: The Two Towers",
    "genres": "Adventure,Drama,Fantasy",
    "directors": "Peter Jackson",
    "cast": " Bruce Allpress as Aldor, Sean Astin as Sam, John Bach as Madril, Sala Baker as Man Flesh Uruk, Cate Blanchett as Galadriel ",
    "rating": "8.7",
    "description": "While Frodo and Sam edge closer to Mordor with the help of the shifty Gollum, the divided fellowship makes a stand against Sauron's new ally, Saruman, and his hordes of Isengard."
  },
  "Sixteen": {
    "title": "One Flew Over the Cuckoo's Nest",
    "genres": "Drama",
    "directors": "Milos Forman",
    "cast": " Michael Berryman as Ellis, Peter Brocco as Col. Matterson, Dean R. Brooks as Dr. Spivey, Alonzo Brown as Miller, Scatman Crothers as Turkle ",
    "rating": "8.7",
    "description": "A criminal pleads insanity after getting into trouble again and once in the mental institution rebels against the oppressive nurse and rallies up the scared patients."
  },
  "Seventeen": {
    "title": "Goodfellas",
    "genres": "Crime,Drama",
    "directors": "Martin Scorsese",
    "cast": " Robert De Niro as James Conway, Ray Liotta as Henry Hill, Joe Pesci as Tommy DeVito, Lorraine Bracco as Karen Hill, Paul Sorvino as Paul Cicero ",
    "rating": "8.7",
    "description": "The story of Henry Hill and his life through the teen years into the years of mafia, covering his relationship with wife Karen Hill and his Mob partners Jimmy Conway and Tommy DeVitto in the Italian-American crime syndicate."
  },
  "Eighteen": {
    "title": "The Matrix",
    "genres": "Action,Sci-Fi",
    "directors": "Lana Wachowski,Lilly Wachowski",
    "cast": " Keanu Reeves as Neo, Laurence Fishburne as Morpheus, Carrie-Anne Moss as Trinity, Hugo Weaving as Agent Smith, Gloria Foster as Oracle ",
    "rating": "8.7",
    "description": "A computer hacker learns from mysterious rebels about the true nature of his reality and his role in the war against its controllers."
  },
  "Nineteen": {
    "title": "Shichinin no samurai",
    "genres": "Adventure,Drama",
    "directors": "Akira Kurosawa",
    "cast": " Toshirô Mifune as Kikuchiyo, Takashi Shimura as Kambei Shimada, Keiko Tsushima as Shino, Yukiko Shimazaki as Wife, Kamatari Fujiwara as Farmer Manzo ",
    "rating": "8.7",
    "description": "A poor village under attack by bandits recruits seven unemployed samurai to help them defend themselves."
  },
  "Twenty": {
    "title": "Star Wars",
    "genres": "Action,Adventure,Fantasy,Sci-Fi",
    "directors": "George Lucas",
    "cast": " Mark Hamill as Luke Skywalker, Harrison Ford as Han Solo, Carrie Fisher as Princess Leia Organa, Peter Cushing as Grand Moff Tarkin, Alec Guinness as Ben Obi-Wan Kenobi ",
    "rating": "8.7",
    "description": "Luke Skywalker joins forces with a Jedi Knight, a cocky pilot, a wookiee and two droids to save the galaxy from the Empire's world-destroying battle-station, while also attempting to rescue Princess Leia from the evil Darth Vader."
  },
  "Twenty One": {
    "title": "Cidade de Deus",
    "genres": "Crime,Drama",
    "directors": "Fernando Meirelles,Kátia Lund",
    "cast": " Alexandre Rodrigues as Buscapé - Rocket, Leandro Firmino as Zé Pequeno - Li'l Zé, Phellipe Haagensen as Bené - Benny, Douglas Silva as Dadinho - Li'l Dice, Jonathan Haagensen as Cabeleira - Shaggy ",
    "rating": "8.7",
    "description": "Two boys growing up in a violent neighborhood of Rio de Janeiro take different paths: one becomes a photographer, the other a drug dealer."
  },
  "Twenty Two": {
    "title": "Se7en",
    "genres": "Crime,Drama,Mystery,Thriller",
    "directors": "David Fincher",
    "cast": " Morgan Freeman as Somerset, Andrew Kevin Walker as Dead Man at 1st Crime Scene, Kevin Spacey as John Doe, Daniel Zacapa as Detective Taylor, Brad Pitt as Mills ",
    "rating": "8.6",
    "description": "Two detectives, a rookie and a veteran, hunt a serial killer who uses the seven deadly sins as his motives."
  },
  "Twenty Three": {
    "title": "The Silence of the Lambs",
    "genres": "Crime,Drama,Thriller",
    "directors": "Jonathan Demme",
    "cast": " Jodie Foster as Clarice Starling, Lawrence A. Bonney as FBI Instructor, Kasi Lemmons as Ardelia Mapp, Lawrence T. Wrentz as Agent Burroughs, Scott Glenn as Jack Crawford ",
    "rating": "8.6",
    "description": "A young F.B.I. cadet must confide in an incarcerated and manipulative killer to receive his help on catching another serial killer who skins his victims."
  },
  "Twenty Four": {
    "title": "It's a Wonderful Life",
    "genres": "Drama,Family,Fantasy",
    "directors": "Frank Capra",
    "cast": " James Stewart as George Bailey, Donna Reed as Mary Hatch, Lionel Barrymore as Mr. Potter, Thomas Mitchell as Uncle Billy, Henry Travers as Clarence ",
    "rating": "8.6",
    "description": "An angel is sent from Heaven to help a desperately frustrated businessman by showing him what life would have been like if he had never existed."
  },
  "Twenty Five": {
    "title": "La vita è bella",
    "genres": "Comedy,Drama,War",
    "directors": "Roberto Benigni",
    "cast": " Roberto Benigni as Guido, Nicoletta Braschi as Dora, Giorgio Cantarini as Giosué, Giustino Durano as Zio, Sergio Bini Bustric as Ferruccio ",
    "rating": "8.6",
    "description": "When an open-minded Jewish librarian and his son become victims of the Holocaust, he uses a perfect mixture of will, humor and imagination to protect his son from the dangers around their camp."
  },
  "Twenty Six": {
    "title": "The Usual Suspects",
    "genres": "Crime,Drama,Mystery,Thriller",
    "directors": "Bryan Singer",
    "cast": " Stephen Baldwin as McManus, Gabriel Byrne as Keaton, Benicio Del Toro as Fenster, Kevin Pollak as Hockney, Kevin Spacey as Verbal ",
    "rating": "8.6",
    "description": "A sole survivor tells of the twisty events leading up to a horrific gun battle on a boat, which began when five criminals met at a seemingly random police lineup."
  },
  "Twenty Seven": {
    "title": "Léon",
    "genres": "Crime,Drama,Thriller",
    "directors": "Luc Besson",
    "cast": " Jean Reno as Léon, Gary Oldman as Stansfield, Natalie Portman as Mathilda, Danny Aiello as Tony, Peter Appel as Malky ",
    "rating": "8.6",
    "description": "Mathilda, a 12-year-old girl, is reluctantly taken in by Léon, a professional assassin, after her family is murdered. Léon and Mathilda form an unusual relationship, as she becomes his protégée and learns the assassin's trade."
  },
  "Twenty Eight": {
    "title": "Sen to Chihiro no kamikakushi",
    "genres": "Animation,Adventure,Family,Fantasy,Mystery",
    "directors": "Hayao Miyazaki",
    "cast": " Rumi Hiiragi as Chihiro Ogino / Sen, Miyu Irino as Haku, Mari Natsuki as Yubaba / Zeniba, Takashi Naitô as Akio Ogino, Yasuko Sawaguchi as Yûko Ogino ",
    "rating": "8.6",
    "description": "During her family's move to the suburbs, a sullen 10-year-old girl wanders into a world ruled by gods, witches, and spirits, and where humans are changed into beasts."
  },
  "Twenty Nine": {
    "title": "Saving Private Ryan",
    "genres": "Drama,War",
    "directors": "Steven Spielberg",
    "cast": " Tom Hanks as Captain Miller, Tom Sizemore as Sergeant Horvath, Edward Burns as Private Reiben, Barry Pepper as Private Jackson, Adam Goldberg as Private Mellish ",
    "rating": "8.6",
    "description": "Following the Normandy Landings, a group of U.S. soldiers go behind enemy lines to retrieve a paratrooper whose brothers have been killed in action."
  },
  "Thirty": {
    "title": "C'era una volta il West",
    "genres": "Western",
    "directors": "Sergio Leone",
    "cast": " Claudia Cardinale as Jill McBain, Henry Fonda as Frank, Jason Robards as Manuel 'Cheyenne' Gutiérrez, Charles Bronson as Harmonica, Gabriele Ferzetti as Morton - Railroad Baron ",
    "rating": "8.6",
    "description": "A mysterious stranger with a harmonica joins forces with a notorious desperado to protect a beautiful widow from a ruthless assassin working for the railroad."
  },
  "Thirty One": {
    "title": "American History X",
    "genres": "Crime,Drama",
    "directors": "Tony Kaye",
    "cast": " Edward Norton as Derek Vinyard, Edward Furlong as Danny Vinyard, Beverly D'Angelo as Doris Vinyard, Jennifer Lien as Davina Vinyard, Ethan Suplee as Seth Ryan ",
    "rating": "8.5",
    "description": "A former neo-nazi skinhead tries to prevent his younger brother from going down the same wrong path that he did."
  },
  "Thirty Two": {
    "title": "Interstellar",
    "genres": "Adventure,Drama,Sci-Fi",
    "directors": "Christopher Nolan",
    "cast": " Ellen Burstyn as Murph, Matthew McConaughey as Cooper, Mackenzie Foy as Murph, John Lithgow as Donald, Timothée Chalamet as Tom ",
    "rating": "8.6",
    "description": "A team of explorers travel through a wormhole in space in an attempt to ensure humanity's survival."
  },
  "Thirty Three": {
    "title": "Casablanca",
    "genres": "Drama,Romance,War",
    "directors": "Michael Curtiz",
    "cast": " Humphrey Bogart as Rick Blaine, Ingrid Bergman as Ilsa Lund, Paul Henreid as Victor Laszlo, Claude Rains as Captain Louis Renault, Conrad Veidt as Major Heinrich Strasser ",
    "rating": "8.5",
    "description": "In Casablanca in December 1941, a cynical American expatriate encounters a former lover, with unforeseen complications."
  },
  "Thirty Four": {
    "title": "Psycho",
    "genres": "Horror,Mystery,Thriller",
    "directors": "Alfred Hitchcock",
    "cast": " Anthony Perkins as Norman Bates, Vera Miles as Lila Crane, John Gavin as Sam Loomis, Janet Leigh as Marion Crane, Martin Balsam as Det. Milton Arbogast ",
    "rating": "8.5",
    "description": "A Phoenix secretary embezzles $40,000 from her employer's client, goes on the run, and checks into a remote motel run by a young man under the domination of his mother."
  },
  "Thirty Five": {
    "title": "City Lights",
    "genres": "Comedy,Drama,Romance",
    "directors": "Charles Chaplin",
    "cast": " Virginia Cherrill as A Blind Girl, Florence Lee as The Blind Girl's Grandmother, Harry Myers as An Eccentric Millionaire, Garcia Al Ernest as James - the Millionaire's Butler, Hank Mann as A Prizefighter ",
    "rating": "8.6",
    "description": "With the aid of a wealthy erratic tippler, a dewy-eyed tramp who has fallen in love with a sightless flower girl accumulates money to be able to help her medically."
  },
  "Thirty Six": {
    "title": "The Green Mile",
    "genres": "Crime,Drama,Fantasy,Mystery",
    "directors": "Frank Darabont",
    "cast": " Tom Hanks as Paul Edgecomb, David Morse as Brutus 'Brutal' Howell, Michael Clarke Duncan as John Coffey, Bonnie Hunt as Jan Edgecomb, James Cromwell as Warden Hal Moores ",
    "rating": "8.5",
    "description": "The lives of guards on Death Row are affected by one of their charges: a black man accused of child murder and rape, yet who has a mysterious gift."
  },
  "Thirty Seven": {
    "title": "Intouchables",
    "genres": "Biography,Comedy,Drama",
    "directors": "Olivier Nakache,Eric Toledano",
    "cast": " François Cluzet as Philippe, Omar Sy as Driss, Anne Le Ny as Yvonne, Audrey Fleurot as Magalie, Joséphine de Meaux as La DRH société de courses ",
    "rating": "8.6",
    "description": "After he becomes a quadriplegic from a paragliding accident, an aristocrat hires a young man from the projects to be his caregiver."
  },
  "Thirty Eight": {
    "title": "Modern Times",
    "genres": "Comedy,Drama,Family,Romance",
    "directors": "Charles Chaplin",
    "cast": " Charles Chaplin as A Factory Worker, Paulette Goddard as A Gamin, Henry Bergman as Cafe Proprietor, Tiny Sandford as Big Bill, Chester Conklin as Mechanic ",
    "rating": "8.5",
    "description": "The Tramp struggles to live in modern industrial society with the help of a young homeless woman."
  },
  "Thirty Nine": {
    "title": "Raiders of the Lost Ark",
    "genres": "Action,Adventure",
    "directors": "Steven Spielberg",
    "cast": " Harrison Ford as Indy, Karen Allen as Marion Ravenwood, Paul Freeman as Dr. René Belloq, Ronald Lacey as Major Arnold Toht, John Rhys-Davies as Sallah ",
    "rating": "8.5",
    "description": "Archaeologist and adventurer Indiana Jones is hired by the U.S. government to find the Ark of the Covenant before the Nazis."
  },
  "Forty": {
    "title": "Rear Window",
    "genres": "Mystery,Thriller",
    "directors": "Alfred Hitchcock",
    "cast": " James Stewart as L.B. 'Jeff' Jefferies, Grace Kelly as Lisa Carol Fremont, Wendell Corey as Det. Lt. Thomas J. Doyle, Thelma Ritter as Stella, Raymond Burr as Lars Thorwald ",
    "rating": "8.5",
    "description": "A wheelchair-bound photographer spies on his neighbours from his apartment window and becomes convinced one of them has committed murder."
  },
  "Forty One": {
    "title": "The Pianist",
    "genres": "Biography,Drama,Music,War",
    "directors": "Roman Polanski",
    "cast": " Adrien Brody as Wladyslaw Szpilman, Emilia Fox as Dorota, Michal Zebrowski as Jurek, Ed Stoppard as Henryk, Maureen Lipman as Mother ",
    "rating": "8.5",
    "description": "A Polish Jewish musician struggles to survive the destruction of the Warsaw ghetto of World War II."
  },
  "Forty Two": {
    "title": "The Departed",
    "genres": "Crime,Drama,Thriller",
    "directors": "Martin Scorsese",
    "cast": " Leonardo DiCaprio as Billy, Matt Damon as Colin Sullivan, Jack Nicholson as Frank Costello, Mark Wahlberg as Dignam, Martin Sheen as Queenan ",
    "rating": "8.5",
    "description": "An undercover cop and a mole in the police attempt to identify each other while infiltrating an Irish gang in South Boston."
  },
  "Forty Three": {
    "title": "Terminator 2: Judgment Day",
    "genres": "Action,Sci-Fi,Thriller",
    "directors": "James Cameron",
    "cast": " Arnold Schwarzenegger as The Terminator, Linda Hamilton as Sarah Connor, Edward Furlong as John Connor, Robert Patrick as T-1000, Earl Boen as Dr. Silberman ",
    "rating": "8.5",
    "description": "A cyborg, identical to the one who failed to kill Sarah Connor, must now protect her teenage son, John Connor, from a more advanced cyborg."
  },
  "Forty Four": {
    "title": "Back to the Future",
    "genres": "Adventure,Comedy,Sci-Fi",
    "directors": "Robert Zemeckis",
    "cast": " Michael J. Fox as Marty McFly, Christopher Lloyd as Dr. Emmett Brown, Lea Thompson as Lorraine Baines, Crispin Glover as George McFly, Thomas F. Wilson as Biff Tannen ",
    "rating": "8.5",
    "description": "Marty McFly, a 17-year-old high school student, is accidentally sent 30 years into the past in a time-traveling DeLorean invented by his close friend, the maverick scientist Doc Brown."
  },
  "Forty Five": {
    "title": "Whiplash",
    "genres": "Drama,Music",
    "directors": "Damien Chazelle",
    "cast": " Miles Teller as Andrew, J.K. Simmons as Fletcher, Paul Reiser as Jim Neimann, Melissa Benoist as Nicole, Austin Stowell as Ryan ",
    "rating": "8.5",
    "description": "A promising young drummer enrolls at a cut-throat music conservatory where his dreams of greatness are mentored by an instructor who will stop at nothing to realize a student's potential."
  },
  "Forty Six": {
    "title": "Gladiator",
    "genres": "Action,Adventure,Drama",
    "directors": "Ridley Scott",
    "cast": " Russell Crowe as Maximus, Joaquin Phoenix as Commodus, Connie Nielsen as Lucilla, Oliver Reed as Proximo, Richard Harris as Marcus Aurelius ",
    "rating": "8.5",
    "description": "When a Roman general is betrayed and his family murdered by an emperor's corrupt son, he comes to Rome as a gladiator to seek revenge."
  },
  "Forty Seven": {
    "title": "Memento",
    "genres": "Mystery,Thriller",
    "directors": "Christopher Nolan",
    "cast": " Guy Pearce as Leonard, Carrie-Anne Moss as Natalie, Joe Pantoliano as Teddy, Mark Boone Junior as Burt, Russ Fega as Waiter ",
    "rating": "8.5",
    "description": "A man juggles searching for his wife's murderer and keeping his short-term memory loss from being an obstacle."
  },
  "Forty Eight": {
    "title": "The Prestige",
    "genres": "Drama,Mystery,Sci-Fi,Thriller",
    "directors": "Christopher Nolan",
    "cast": " Hugh Jackman as Robert Angier, Christian Bale as Alfred Borden, Michael Caine as Cutter, Piper Perabo as Julia McCullough, Rebecca Hall as Sarah ",
    "rating": "8.5",
    "description": "Two stage magicians engage in competitive one-upmanship in an attempt to create the ultimate stage illusion."
  },
  "Forty Nine": {
    "title": "The Lion King",
    "genres": "Animation,Adventure,Drama,Family,Musical",
    "directors": "Roger Allers,Rob Minkoff",
    "cast": " Rowan Atkinson as Zazu, Matthew Broderick as Simba, Niketa Calame as Young Nala, Jim Cummings as Ed, Whoopi Goldberg as Shenzi ",
    "rating": "8.5",
    "description": "Lion cub and future king Simba searches for his identity. His eagerness to please others and penchant for testing his boundaries sometimes gets him into trouble."
  },
  "Fifty": {
    "title": "Apocalypse Now",
    "genres": "Drama,War",
    "directors": "Francis Ford Coppola",
    "cast": " Marlon Brando as Colonel Walter E. Kurtz, Martin Sheen as Captain Benjamin L. Willard, Robert Duvall as Lieutenant Colonel Bill Kilgore, Frederic Forrest as Jay 'Chef' Hicks, Sam Bottoms as Lance B. Johnson ",
    "rating": "8.5",
    "description": "During the Vietnam War, Captain Willard is sent on a dangerous mission into Cambodia to assassinate a renegade colonel who has set himself up as a god among a local tribe."
  },
  "Fifty One": {
    "title": "Alien",
    "genres": "Horror,Sci-Fi",
    "directors": "Ridley Scott",
    "cast": " Tom Skerritt as Dallas, Sigourney Weaver as Ripley, Veronica Cartwright as Lambert, Harry Dean Stanton as Brett, John Hurt as Kane ",
    "rating": "8.5",
    "description": "After a space merchant vessel perceives an unknown transmission as a distress call, its landing on the source moon finds one of the crew attacked by a mysterious life-form, and they soon realize that its life cycle has merely begun."
  },
  "Fifty Two": {
    "title": "Sunset Blvd.",
    "genres": "Drama,Film-Noir",
    "directors": "Billy Wilder",
    "cast": " William Holden as Joe Gillis, Gloria Swanson as Norma Desmond, Erich von Stroheim as Max Von Mayerling, Nancy Olson as Betty Schaefer, Fred Clark as Sheldrake ",
    "rating": "8.5",
    "description": "A screenwriter is hired to rework a faded silent film star's script only to find himself developing a dangerous relationship."
  },
  "Fifty Three": {
    "title": "The Great Dictator",
    "genres": "Comedy,Drama,War",
    "directors": "Charles Chaplin",
    "cast": " Charles Chaplin as Hynkel - Dictator of Tomania / A Jewish Barber, Jack Oakie as Napaloni - Dictator of Bacteria, Reginald Gardiner as Schultz, Henry Daniell as Garbitsch, Billy Gilbert as Herring ",
    "rating": "8.5",
    "description": "Dictator Adenoid Hynkel tries to expand his empire while a poor Jewish barber tries to avoid persecution from Hynkel's regime."
  },
  "Fifty Four": {
    "title": "Dr. Strangelove or: How I Learned to Stop Worrying and Love the Bomb",
    "genres": "Comedy",
    "directors": "Stanley Kubrick",
    "cast": " Peter Sellers as Group Capt. Lionel Mandrake / President Merkin Muffley / Dr. Strangelove, George C. Scott as Gen. 'Buck' Turgidson, Sterling Hayden as Brig. Gen. Jack D. Ripper, Keenan Wynn as Col. 'Bat' Guano, Slim Pickens as Maj. 'King' Kong ",
    "rating": "8.5",
    "description": "An insane general triggers a path to nuclear holocaust that a war room full of politicians and generals frantically try to stop."
  },
  "Fifty Five": {
    "title": "Nuovo Cinema Paradiso",
    "genres": "Drama",
    "directors": "Giuseppe Tornatore",
    "cast": " Antonella Attili as Maria Di Vita - Younger, Enzo Cannavale as Spaccafico, Isa Danieli as Anna, Leo Gullotta as Usher, Marco Leonardi as Salvatore 'Totò' Di Vita - Teenager ",
    "rating": "8.5",
    "description": "A filmmaker recalls his childhood, when he fell in love with the movies at his village's theater and formed a deep friendship with the theater's projectionist."
  },
  "Fifty Six": {
    "title": "Das Leben der Anderen",
    "genres": "Drama,Thriller",
    "directors": "Florian Henckel von Donnersmarck",
    "cast": " Martina Gedeck as Christa-Maria Sieland, Ulrich Mühe as Hauptmann Gerd Wiesler, Sebastian Koch as Georg Dreyman, Ulrich Tukur as Oberstleutnant Anton Grubitz, Thomas Thieme as Minister Bruno Hempf ",
    "rating": "8.5",
    "description": "In 1984 East Berlin, an agent of the secret police, conducting surveillance on a writer and his lover, finds himself becoming increasingly absorbed by their lives."
  },
  "Fifty Seven": {
    "title": "Hotaru no haka",
    "genres": "Animation,Drama,War",
    "directors": "Isao Takahata",
    "cast": " Tsutomu Tatsumi as Seita, Ayano Shiraishi as Setsuko, Yoshiko Shinohara as Mother, Akemi Yamaguchi as Aunt, Tadashi Nakamura as  ",
    "rating": "8.5",
    "description": "A young boy and his little sister struggle to survive in Japan during World War II."
  },
  "Fifty Eight": {
    "title": "Paths of Glory",
    "genres": "Drama,War",
    "directors": "Stanley Kubrick",
    "cast": " Kirk Douglas as Col. Dax, Ralph Meeker as Cpl. Philippe Paris, Adolphe Menjou as Gen. George Broulard, George Macready as Gen. Paul Mireau, Wayne Morris as Lt. Roget ",
    "rating": "8.5",
    "description": "After refusing to attack an enemy position, a general accuses the soldiers of cowardice and their commanding officer must defend them."
  },
  "Fifty Nine": {
    "title": "Django Unchained",
    "genres": "Drama,Western",
    "directors": "Quentin Tarantino",
    "cast": " Jamie Foxx as Django, Christoph Waltz as Dr. King Schultz, Leonardo DiCaprio as Calvin Candie, Kerry Washington as Broomhilda von Shaft, Samuel L. Jackson as Stephen ",
    "rating": "8.4",
    "description": "With the help of a German bounty hunter , a freed slave sets out to rescue his wife from a brutal Mississippi plantation owner."
  },
  "Sixty": {
    "title": "The Shining",
    "genres": "Drama,Horror",
    "directors": "Stanley Kubrick",
    "cast": " Jack Nicholson as Jack Torrance, Shelley Duvall as Wendy Torrance, Danny Lloyd as Danny, Scatman Crothers as Hallorann, Barry Nelson as Ullman ",
    "rating": "8.4",
    "description": "A family heads to an isolated hotel for the winter where an evil and spiritual presence influences the father into violence, while his psychic son sees horrific forebodings from the past and of the future."
  },
  "Sixty One": {
    "title": "WALL·E",
    "genres": "Animation,Adventure,Family,Sci-Fi",
    "directors": "Andrew Stanton",
    "cast": " Ben Burtt as WALL·E / M-O / Robots, Elissa Knight as EVE, Jeff Garlin as Captain, Fred Willard as Shelby Forthright, MacInTalk as AUTO ",
    "rating": "8.4",
    "description": "In the distant future, a small waste-collecting robot inadvertently embarks on a space journey that will ultimately decide the fate of mankind."
  },
  "Sixty Two": {
    "title": "American Beauty",
    "genres": "Drama,Romance",
    "directors": "Sam Mendes",
    "cast": " Kevin Spacey as Lester Burnham, Annette Bening as Carolyn Burnham, Thora Birch as Jane Burnham, Wes Bentley as Ricky Fitts, Mena Suvari as Angela Hayes ",
    "rating": "8.4",
    "description": "A sexually frustrated suburban father has a mid-life crisis after becoming infatuated with his daughter's best friend."
  },
  "Sixty Three": {
    "title": "The Dark Knight Rises",
    "genres": "Action,Thriller",
    "directors": "Christopher Nolan",
    "cast": " Christian Bale as Bruce Wayne, Gary Oldman as Commissioner Gordon, Tom Hardy as Bane, Joseph Gordon-Levitt as Blake, Anne Hathaway as Selina ",
    "rating": "8.5",
    "description": "Eight years after the Joker's reign of anarchy, the Dark Knight, with the help of the enigmatic Catwoman, is forced from his exile to save Gotham City, now on the edge of total annihilation, from the brutal guerrilla terrorist Bane."
  },
  "Sixty Four": {
    "title": "Mononoke-hime",
    "genres": "Animation,Adventure,Fantasy",
    "directors": "Hayao Miyazaki",
    "cast": " Billy Crudup as Ashitaka, Billy Bob Thornton as Jigo, Minnie Driver as Lady Eboshi, John DiMaggio as Gonza, Claire Danes as San ",
    "rating": "8.4",
    "description": "On a journey to find the cure for a Tatarigami's curse, Ashitaka finds himself in the middle of a war between the forest gods and Tatara, a mining colony. In this quest he also meets San, the Mononoke Hime."
  },
  "Sixty Five": {
    "title": "Aliens",
    "genres": "Action,Adventure,Sci-Fi,Thriller",
    "directors": "James Cameron",
    "cast": " Sigourney Weaver as Ripley, Carrie Henn as Newt, Michael Biehn as Corporal Hicks, Paul Reiser as Burke, Lance Henriksen as Bishop ",
    "rating": "8.4",
    "description": "The moon from  (1979) has been colonized, but contact is lost. This time, the rescue team has impressive firepower, but will it be enough?"
  },
  "Sixty Six": {
    "title": "Oldeuboi",
    "genres": "Drama,Mystery,Thriller",
    "directors": "Chan-wook Park",
    "cast": " Min-sik Choi as Dae-su Oh, Ji-tae Yu as Woo-jin Lee, Hye-jeong Kang as Mi-do, Dae-han Ji as No Joo-hwan, Dal-su Oh as Park Cheol-woong ",
    "rating": "8.4",
    "description": "After being kidnapped and imprisoned for fifteen years, Oh Dae-Su is released, only to find that he must find his captor in five days."
  },
  "Sixty Seven": {
    "title": "Once Upon a Time in America",
    "genres": "Crime,Drama",
    "directors": "Sergio Leone",
    "cast": " Robert De Niro as David 'Noodles' Aaronson, James Woods as Maximilian 'Max' Bercovicz, Elizabeth McGovern as Deborah Gelly, Joe Pesci as Frankie Manoldi, Burt Young as Joe ",
    "rating": "8.4",
    "description": "A former Prohibition-era Jewish gangster returns to the Lower East Side of Manhattan over thirty years later, where he once again must confront the ghosts and regrets of his old life."
  },
  "Sixty Eight": {
    "title": "Witness for the Prosecution",
    "genres": "Crime,Drama,Mystery,Thriller",
    "directors": "Billy Wilder",
    "cast": " Tyrone Power as Leonard Vole, Marlene Dietrich as Christine, Charles Laughton as Sir Wilfrid Roberts, Elsa Lanchester as Miss Plimsoll, John Williams as Brogan-Moore ",
    "rating": "8.4",
    "description": "A veteran British barrister must defend his client in a murder trial that has surprise after surprise."
  },
  "Sixty Nine": {
    "title": "Das Boot",
    "genres": "Adventure,Drama,Thriller,War",
    "directors": "Wolfgang Petersen",
    "cast": " Jürgen Prochnow as Capt.-Lt. Henrich Lehmann-Willenbrock - Der Alte, Herbert Grönemeyer as Lt. Werner - Correspondent, Klaus Wennemann as Chief Engineer Fritz Grade - Der Leitende-Der LI, Hubertus Bengsch as 1st Lieutenant - Number One-1WO, Martin Semmelrogge as 2nd Lieutenant - 2WO ",
    "rating": "8.4",
    "description": "The claustrophobic world of a WWII German U-boat; boredom, filth, and sheer terror."
  },
  "Seventy": {
    "title": "Citizen Kane",
    "genres": "Drama,Mystery",
    "directors": "Orson Welles",
    "cast": " Joseph Cotten as Jedediah Leland / Screening Room Reporter, Dorothy Comingore as Susan Alexander Kane, Agnes Moorehead as Mary Kane, Ruth Warrick as Emily Monroe Norton Kane, Ray Collins as James W. Gettys ",
    "rating": "8.4",
    "description": "Following the death of a publishing tycoon, news reporters scramble to discover the meaning of his final utterance."
  },
  "Seventy One": {
    "title": "North by Northwest",
    "genres": "Action,Adventure,Mystery,Thriller",
    "directors": "Alfred Hitchcock",
    "cast": " Cary Grant as Roger O. Thornhill, Eva Marie Saint as Eve Kendall, James Mason as Phillip Vandamm, Jessie Royce Landis as Clara Thornhill, Leo G. Carroll as The Professor ",
    "rating": "8.4",
    "description": "A hapless New York advertising executive is mistaken for a government agent by a group of foreign spies, and is pursued across the country while he looks for a way to survive."
  },
  "Seventy Two": {
    "title": "Vertigo",
    "genres": "Mystery,Romance,Thriller",
    "directors": "Alfred Hitchcock",
    "cast": " James Stewart as John 'Scottie' Ferguson, Kim Novak as Madeleine Elster / Judy Barton, Barbara Bel Geddes as Midge Wood, Tom Helmore as Gavin Elster, Henry Jones as Coroner ",
    "rating": "8.4",
    "description": "A San Francisco detective suffering from acrophobia investigates the strange activities of an old friend's wife, all the while becoming dangerously obsessed with her."
  },
  "Seventy Three": {
    "title": "Star Wars: Episode VI - Return of the Jedi",
    "genres": "Action,Adventure,Fantasy,Sci-Fi",
    "directors": "Richard Marquand",
    "cast": " Mark Hamill as Luke Skywalker, Harrison Ford as Han Solo, Carrie Fisher as Princess Leia, Billy Dee Williams as Lando Calrissian, Anthony Daniels as C-3PO ",
    "rating": "8.4",
    "description": "After rescuing Han Solo from the palace of Jabba the Hutt, the rebels attempt to destroy the second Death Star, while Luke struggles to make Vader return from the dark side of the Force."
  },
  "Seventy Four": {
    "title": "Dangal",
    "genres": "Action,Biography,Drama,Sport",
    "directors": "Nitesh Tiwari",
    "cast": " Aamir Khan as Mahavir Singh Phogat, Sakshi Tanwar as Daya Kaur, Fatima Sana Shaikh as Geeta Phogat, Sanya Malhotra as Babita Kumari, Aparshakti Khurana as Omkar ",
    "rating": "8.7",
    "description": "Former wrestler Mahavir Singh Phogat and his two wrestler daughters struggle towards glory at the Commonwealth Games in the face of societal oppression."
  },
  "Seventy Five": {
    "title": "Reservoir Dogs",
    "genres": "Crime,Drama,Thriller",
    "directors": "Quentin Tarantino",
    "cast": " Harvey Keitel as Mr. White - Larry Dimmick, Tim Roth as Mr. Orange - Freddy Newandyke, Michael Madsen as Mr. Blonde - Vic Vega, Chris Penn as Nice Guy Eddie Cabot, Steve Buscemi as Mr. Pink ",
    "rating": "8.3",
    "description": "After a simple jewelry heist goes terribly wrong, the surviving criminals begin to suspect that one of them is a police informant."
  },
  "Seventy Six": {
    "title": "Braveheart",
    "genres": "Biography,Drama,History,War",
    "directors": "Mel Gibson",
    "cast": " James Robinson as Young William, Sean Lawlor as Malcolm Wallace, Sandy Nelson as John Wallace, James Cosmo as Campbell, Sean McGinley as MacClannough ",
    "rating": "8.4",
    "description": "When his secret bride is executed for assaulting an English soldier who tried to rape her, Sir William Wallace begins a revolt against King Edward I of England."
  },
  "Seventy Seven": {
    "title": "M",
    "genres": "Crime,Drama,Mystery,Thriller",
    "directors": "Fritz Lang",
    "cast": " Peter Lorre as Hans Beckert, Ellen Widmann as Frau Beckmann, Inge Landgut as Elsie Beckmann, Otto Wernicke as Inspector Karl Lohmann, Theodor Loos as Inspector Groeber ",
    "rating": "8.4",
    "description": "When the police in a German city are unable to catch a child-murderer, other criminals join in the manhunt."
  },
  "Seventy Eight": {
    "title": "Requiem for a Dream",
    "genres": "Drama",
    "directors": "Darren Aronofsky",
    "cast": " Ellen Burstyn as Sara Goldfarb, Jared Leto as Harry Goldfarb, Jennifer Connelly as Marion Silver, Marlon Wayans as Tyrone C. Love, Christopher McDonald as Tappy Tibbons ",
    "rating": "8.4",
    "description": "The drug-induced utopias of four Coney Island people are shattered when their addictions run deep."
  },
  "Seventy Nine": {
    "title": "Le fabuleux destin d'Amélie Poulain",
    "genres": "Comedy,Romance",
    "directors": "Jean-Pierre Jeunet",
    "cast": " Audrey Tautou as Amélie Poulain, Mathieu Kassovitz as Nino Quincampoix, Rufus as Raphaël Poulain, Lorella Cravotta as Amandine Poulain, Serge Merlin as Raymond Dufayel ",
    "rating": "8.4",
    "description": "Amélie is an innocent and naive girl in Paris with her own sense of justice. She decides to help those around her and, along the way, discovers love."
  },
  "Eighty": {
    "title": "Taare Zameen Par",
    "genres": "Drama,Family,Music",
    "directors": "Aamir Khan,Amole Gupte",
    "cast": " Darsheel Safary as Ishaan Awasthi, Aamir Khan as Ram Shankar Nikumbh, Tanay Chheda as Rajan Damodaran, Sachet Engineer as Yohan Awasthi, Tisca Chopra as Maya Awasthi ",
    "rating": "8.5",
    "description": "An eight-year-old boy is thought to be a lazy trouble-maker, until the new art teacher has the patience and compassion to discover the real problem behind his struggles in school."
  },
  "Eighty One": {
    "title": "A Clockwork Orange",
    "genres": "Crime,Drama,Sci-Fi",
    "directors": "Stanley Kubrick",
    "cast": " Malcolm McDowell as Alex, Patrick Magee as Mr. Alexander, Michael Bates as Chief Guard, Warren Clarke as Dim, John Clive as Stage Actor ",
    "rating": "8.3",
    "description": "In future Britain, Alex DeLarge, a charismatic and psycopath delinquent, who likes to practice crimes and ultra-violence with his gang, is jailed and volunteers for an experimental aversion therapy developed by the government in an effort to solve society's crime problem - but not all goes according to plan."
  },
  "Eighty Two": {
    "title": "Lawrence of Arabia",
    "genres": "Adventure,Biography,Drama,History,War",
    "directors": "David Lean",
    "cast": " Peter O'Toole as T.E. Lawrence, Alec Guinness as Prince Feisal, Anthony Quinn as Auda Abu Tayi, Jack Hawkins as General Allenby, Omar Sharif as Sherif Ali ",
    "rating": "8.3",
    "description": "The story of , the English officer who successfully united and led the diverse, often warring, Arab tribes during World War I in order to fight the Turks."
  },
  "Eighty Three": {
    "title": "Taxi Driver",
    "genres": "Crime,Drama",
    "directors": "Martin Scorsese",
    "cast": " Diahnne Abbott as Concession Girl, Frank Adu as Angry Black Policeman, Victor Argo as Melio, Gino Ardito as Policeman At Rally, Garth Avery as Iris' Friend ",
    "rating": "8.3",
    "description": "A mentally unstable Vietnam War veteran works as a night-time taxi driver in New York City where the perceived decadence and sleaze feeds his urge for violent action, while attempting to save a preadolescent prostitute in the process."
  },
  "Eighty Four": {
    "title": "Double Indemnity",
    "genres": "Crime,Drama,Film-Noir,Mystery,Thriller",
    "directors": "Billy Wilder",
    "cast": " Fred MacMurray as Walter Neff, Barbara Stanwyck as Phyllis Dietrichson, Edward G. Robinson as Barton Keyes, Porter Hall as Mr. Jackson, Jean Heather as Lola Dietrichson ",
    "rating": "8.3",
    "description": "An insurance representative lets himself be talked into a murder/insurance fraud scheme that arouses an insurance investigator's suspicions."
  },
  "Eighty Five": {
    "title": "Eternal Sunshine of the Spotless Mind",
    "genres": "Drama,Romance,Sci-Fi",
    "directors": "Michel Gondry",
    "cast": " Jim Carrey as Joel Barish, Kate Winslet as Clementine Kruczynski, Gerry Robert Byrne as Train Conductor, Elijah Wood as Patrick, Thomas Jay Ryan as Frank ",
    "rating": "8.3",
    "description": "When their relationship turns sour, a couple undergoes a procedure to have each other erased from their memories. But it is only through the process of loss that they discover what they had to begin with."
  },
  "Eighty Six": {
    "title": "Amadeus",
    "genres": "Biography,Drama,History,Music",
    "directors": "Milos Forman",
    "cast": " F. Murray Abraham as Antonio Salieri, Tom Hulce as Wolfgang Amadeus Mozart, Elizabeth Berridge as Constanze Mozart, Roy Dotrice as Leopold Mozart, Simon Callow as Emanuel Schikaneder ",
    "rating": "8.3",
    "description": "The incredible story of , told by his peer and secret rival  - now confined to an insane asylum."
  },
  "Eighty Seven": {
    "title": "To Kill a Mockingbird",
    "genres": "Crime,Drama",
    "directors": "Robert Mulligan",
    "cast": " Gregory Peck as Atticus Finch, John Megna as Dill Harris, Frank Overton as Sheriff Heck Tate, Rosemary Murphy as Maudie Atkinson, Ruth White as Mrs. Dubose ",
    "rating": "8.3",
    "description": "Atticus Finch, a lawyer in the Depression-era South, defends a black man against an undeserved rape charge, and his children against prejudice."
  },
  "Eighty Eight": {
    "title": "Toy Story 3",
    "genres": "Animation,Adventure,Comedy,Family,Fantasy",
    "directors": "Lee Unkrich",
    "cast": " Tom Hanks as Woody, Tim Allen as Buzz Lightyear, Joan Cusack as Jessie, Ned Beatty as Lotso, Don Rickles as Mr. Potato Head ",
    "rating": "8.3",
    "description": "The toys are mistakenly delivered to a day-care center instead of the attic right before Andy leaves for college, and it's up to Woody to convince the other toys that they weren't abandoned and to return home."
  },
  "Eighty Nine": {
    "title": "Full Metal Jacket",
    "genres": "Drama,War",
    "directors": "Stanley Kubrick",
    "cast": " Matthew Modine as Pvt. J.T. 'Joker' Davis, Adam Baldwin as Animal Mother, Vincent D'Onofrio as Pvt. Leonard 'Gomer Pyle' Lawrence, R. Lee Ermey as Gny. Sgt. Hartman, Dorian Harewood as Eightball ",
    "rating": "8.3",
    "description": "A pragmatic U.S. Marine observes the dehumanizing effects the Vietnam War has on his fellow recruits from their brutal boot camp training to the bloody street fighting in Hue."
  },
  "Ninty": {
    "title": "2001: A Space Odyssey",
    "genres": "Adventure,Sci-Fi",
    "directors": "Stanley Kubrick",
    "cast": " Keir Dullea as Dr. Dave Bowman, Gary Lockwood as Dr. Frank Poole, William Sylvester as Dr. Heywood R. Floyd, Daniel Richter as Moon-Watcher, Leonard Rossiter as Dr. Andrei Smyslov ",
    "rating": "8.3",
    "description": "Humanity finds a mysterious, obviously artificial object buried beneath the Lunar surface and, with the intelligent computer H.A.L. 9000, sets off on a quest."
  },
  "Ninty One": {
    "title": "Singin' in the Rain",
    "genres": "Comedy,Musical,Romance",
    "directors": "Stanley Donen,Gene Kelly",
    "cast": " Gene Kelly as Don Lockwood, Donald O'Connor as Cosmo Brown, Debbie Reynolds as Kathy Selden, Jean Hagen as Lina Lamont, Millard Mitchell as R.F. Simpson ",
    "rating": "8.3",
    "description": "A silent film production company and cast make a difficult transition to sound."
  },
  "Ninty Two": {
    "title": "The Sting",
    "genres": "Comedy,Crime,Drama",
    "directors": "George Roy Hill",
    "cast": " Paul Newman as Henry Gondorff, Robert Redford as Johnny Hooker, Robert Shaw as Doyle Lonnegan, Charles Durning as Lt. Wm. Snyder, Ray Walston as J.J. Singleton ",
    "rating": "8.3",
    "description": "In Chicago in September 1936, a young con man seeking revenge for his murdered partner teams up with a master of the big con to win a fortune from a criminal banker."
  },
  "Ninty Three": {
    "title": "Toy Story",
    "genres": "Animation,Adventure,Comedy,Family,Fantasy",
    "directors": "John Lasseter",
    "cast": " Tom Hanks as Woody, Tim Allen as Buzz Lightyear, Don Rickles as Mr. Potato Head, Jim Varney as Slinky Dog, Wallace Shawn as Rex ",
    "rating": "8.3",
    "description": "A cowboy doll is profoundly threatened and jealous when a new spaceman figure supplants him as top toy in a boy's room."
  },
  "Ninty Four": {
    "title": "Ladri di biciclette",
    "genres": "Drama",
    "directors": "Vittorio De Sica",
    "cast": " Lamberto Maggiorani as Antonio, Enzo Staiola as Bruno, Lianella Carell as Maria, Elena Altieri as The Charitable Lady, Gino Saltamerenda as Baiocco ",
    "rating": "8.3",
    "description": "In post-war Italy, a working-class man's bicycle is stolen. He and his son set out to find it."
  },
  "Ninty Five": {
    "title": "The Kid",
    "genres": "Comedy,Drama,Family",
    "directors": "Charles Chaplin",
    "cast": " Carl Miller as The Man, Edna Purviance as The Woman, Jackie Coogan as The Child, Charles Chaplin as A Tramp, Albert Austin as Man in Shelter ",
    "rating": "8.3",
    "description": "The Tramp cares for an abandoned child, but events put that relationship in jeopardy."
  },
  "Ninty Six": {
    "title": "Inglourious Basterds",
    "genres": "Adventure,Drama,War",
    "directors": "Quentin Tarantino,Eli Roth",
    "cast": " Brad Pitt as Lt. Aldo Raine, Mélanie Laurent as Shosanna, Christoph Waltz as Col. Hans Landa, Eli Roth as Sgt. Donny Donowitz, Michael Fassbender as Lt. Archie Hicox ",
    "rating": "8.3",
    "description": "In Nazi-occupied France during World War II, a plan to assassinate Nazi leaders by a group of Jewish U.S. soldiers coincides with a theatre owner's vengeful plans for the same."
  },
  "Ninty Seven": {
    "title": "Kimi no na wa",
    "genres": "Animation,Drama,Fantasy,Romance",
    "directors": "Makoto Shinkai",
    "cast": " Ryûnosuke Kamiki as Taki Tachibana, Mone Kamishiraishi as Mitsuha Miyamizu, Ryô Narita as Katsuhiko Teshigawara, Aoi Yûki as Sayaka Natori, Nobunaga Shimazaki as Tsukasa Fujii ",
    "rating": "8.6",
    "description": "Two strangers find themselves linked in a bizarre way. When a connection forms, will distance be the only thing to keep them apart?"
  },
  "Ninty Eight": {
    "title": "3 Idiots",
    "genres": "Adventure,Comedy,Drama",
    "directors": "Rajkumar Hirani",
    "cast": " Aamir Khan as 'Rancho' Shamaldas Chanchad, Madhavan as Farhan Qureshi, Sharman Joshi as Raju Rastogi, Kareena Kapoor Khan as Pia, Boman Irani as Viru Sahastrabudhhe ",
    "rating": "8.4",
    "description": "Two friends are searching for their long lost companion. They revisit their college days and recall the memories of their friend who inspired them to think differently, even as the rest of the world called them idiots."
  },
  "Ninty Nine": {
    "title": "Snatch",
    "genres": "Comedy,Crime",
    "directors": "Guy Ritchie",
    "cast": " Benicio Del Toro as Franky Four Fingers, Dennis Farina as Cousin Avi, Vinnie Jones as Bullet-Tooth Tony, Brad Pitt as Mickey O'Neil, Rade Serbedzija as Boris the Blade ",
    "rating": "8.3",
    "description": "Unscrupulous boxing promoters, violent bookmakers, a Russian gangster, incompetent amateur robbers, and supposedly Jewish jewelers fight to track down a priceless stolen diamond."
  },
  "One Hundred": {
    "title": "Monty Python and the Holy Grail",
    "genres": "Adventure,Comedy,Fantasy",
    "directors": "Terry Gilliam,Terry Jones",
    "cast": " Graham Chapman as King Arthur / Voice of God / Middle Head / Hiccoughing Guard, John Cleese as Second Swallow-Savvy Guard / The Black Knight / Peasant 3 / Sir Lancelot the Brave / Taunting French Guard / Tim the Enchanter, Eric Idle as Dead Collector / Peasant 1 / Sir Robin the Not-Quite-So-Brave-as-Sir Launcelot / First Swamp Castle Guard / Concorde / Roger the Shrubber / Brother Maynard, Terry Gilliam as Patsy / Green Knight / Old Man from Scene 24 / Sir Bors / Animator / Gorrilla Hand, Terry Jones as Dennis's Mother / Sir Bedevere / Left Head / Prince Herbert / Cartoon Scribe ",
    "rating": "8.3",
    "description": "King Arthur and his knights embark on a low-budget search for the Grail, encountering many, very silly obstacles."
  },
  "One Hundred One": {
    "title": "L.A. Confidential",
    "genres": "Crime,Drama,Mystery,Thriller",
    "directors": "Curtis Hanson",
    "cast": " Kevin Spacey as Jack Vincennes, Russell Crowe as Bud White, Guy Pearce as Ed Exley, James Cromwell as Dudley Smith, Kim Basinger as Lynn Bracken ",
    "rating": "8.3",
    "description": "As corruption grows in 1950s LA, three policemen - one strait-laced, one brutal, and one sleazy - investigate a series of murders with their own brand of justice."
  },
  "One Hundred Two": {
    "title": "Per qualche dollaro in più",
    "genres": "Western",
    "directors": "Sergio Leone",
    "cast": " Clint Eastwood as Monco, Lee Van Cleef as Col. Douglas Mortimer, Gian Maria Volontè as El Indio, Mario Brega as Member of Indio's Gang Nino, Luigi Pistilli as Member of Indio's Gang Groggy ",
    "rating": "8.3",
    "description": "Two bounty hunters with the same intentions team up to track down a Western outlaw."
  },
  "One Hundred Three": {
    "title": "Babam ve Oglum",
    "genres": "Drama",
    "directors": "Çagan Irmak",
    "cast": " Resit Kurt as Baris, Fikret Kuskan as Sadik, Çetin Tekindor as Hüseyin, Furkan Turan as Kerem, Hümeyra as Nuran ",
    "rating": "8.6",
    "description": "The family of a left-wing journalist is torn apart after a military coup in 1980's Turkey."
  },
  "One Hundred Four": {
    "title": "Jagten",
    "genres": "Drama",
    "directors": "Thomas Vinterberg",
    "cast": " Mads Mikkelsen as Lucas, Thomas Bo Larsen as Theo, Annika Wedderkopp as Klara, Lasse Fogelstrøm as Marcus, Susse Wold as Grethe ",
    "rating": "8.3",
    "description": "A teacher lives a lonely life, all the while struggling over his son's custody. His life slowly gets better as he finds love and receives good news from his son, but his new luck is about to be brutally shattered by an innocent little lie."
  },
  "One Hundred Five": {
    "title": "Scarface",
    "genres": "Crime,Drama",
    "directors": "Brian De Palma",
    "cast": " Al Pacino as Tony Montana, Steven Bauer as Manny Ribera, Michelle Pfeiffer as Elvira Hancock, Mary Elizabeth Mastrantonio as Gina Montana, Robert Loggia as Frank Lopez ",
    "rating": "8.3",
    "description": "In Miami in 1980, a determined Cuban immigrant takes over a drug cartel and succumbs to greed."
  },
  "One Hundred Six": {
    "title": "Good Will Hunting",
    "genres": "Drama",
    "directors": "Gus Van Sant",
    "cast": " Matt Damon as Will, Ben Affleck as Chuckie, Stellan Skarsgård as Lambeau, John Mighton as Tom, Rachel Majorowski as Krystyn ",
    "rating": "8.3",
    "description": "Will Hunting, a janitor at M.I.T., has a gift for mathematics, but needs help from a psychologist to find direction in his life."
  },
  "One Hundred Seven": {
    "title": "The Apartment",
    "genres": "Comedy,Drama,Romance",
    "directors": "Billy Wilder",
    "cast": " Jack Lemmon as C.C. Baxter, Shirley MacLaine as Fran Kubelik, Fred MacMurray as Jeff D. Sheldrake, Ray Walston as Joe Dobisch, Jack Kruschen as Dr. Dreyfuss ",
    "rating": "8.3",
    "description": "A man tries to rise in his company by letting its executives use his apartment for trysts, but complications and a romance of his own ensue."
  },
  "One Hundred Eight": {
    "title": "Rashômon",
    "genres": "Crime,Drama,Mystery",
    "directors": "Akira Kurosawa",
    "cast": " Toshirô Mifune as Tajômaru, Machiko Kyô as Masako Kanazawa, Masayuki Mori as Takehiro Kanazawa, Takashi Shimura as Woodcutter, Minoru Chiaki as Priest ",
    "rating": "8.3",
    "description": "A heinous crime and its aftermath are recalled from differing points of view."
  },
  "One Hundred Nine": {
    "title": "Jodaeiye Nader az Simin",
    "genres": "Drama,Mystery",
    "directors": "Asghar Farhadi",
    "cast": " Peyman Moaadi as Nader, Leila Hatami as Simin, Sareh Bayat as Razieh, Shahab Hosseini as Hojjat, Sarina Farhadi as Termeh ",
    "rating": "8.4",
    "description": "A married couple are faced with a difficult decision - to improve the life of their child by moving to another country or to stay in Iran and look after a deteriorating parent who has Alzheimer's disease."
  },
  "One Hundred Ten": {
    "title": "Metropolis",
    "genres": "Drama,Sci-Fi",
    "directors": "Fritz Lang",
    "cast": " Alfred Abel as Joh Fredersen, Gustav Fröhlich as Freder - Joh Fredersen's Son, Rudolf Klein-Rogge as C.A. Rotwang - the Inventor, Fritz Rasp as The Thin Man, Theodor Loos as Josaphat ",
    "rating": "8.3",
    "description": "In a futuristic city sharply divided between the working class and the city planners, the son of the city's mastermind falls in love with a working class prophet who predicts the coming of a savior to mediate their differences."
  },
  "One Hundred Eleven": {
    "title": "Indiana Jones and the Last Crusade",
    "genres": "Action,Adventure,Fantasy",
    "directors": "Steven Spielberg",
    "cast": " Harrison Ford as Indiana Jones, Sean Connery as Professor Henry Jones, Denholm Elliott as Marcus Brody, Alison Doody as Elsa, John Rhys-Davies as Sallah ",
    "rating": "8.3",
    "description": "When Dr. Henry Jones Sr. suddenly goes missing while pursuing the Holy Grail, eminent archaeologist Indiana Jones must follow in his father's footsteps to stop the Nazis from getting their hands on the Holy Grail first."
  },
  "One Hundred Twelve": {
    "title": "All About Eve",
    "genres": "Drama",
    "directors": "Joseph L. Mankiewicz",
    "cast": " Bette Davis as Margo Channing, Anne Baxter as Eve Harrington, George Sanders as Addison DeWitt, Celeste Holm as Karen Richards, Gary Merrill as Bill Simpson ",
    "rating": "8.3",
    "description": "An ingenue insinuates herself into the company of an established but aging stage actress and her circle of theater friends."
  },
  "One Hundred Thirteen": {
    "title": "Yôjinbô",
    "genres": "Action,Drama,Thriller",
    "directors": "Akira Kurosawa",
    "cast": " Toshirô Mifune as Sanjuro Kuwabatake / The Samurai, Tatsuya Nakadai as Unosuke - Gunfighter, Yôko Tsukasa as Nui, Isuzu Yamada as Orin, Daisuke Katô as Inokichi - Ushitora's Rotund Brother ",
    "rating": "8.3",
    "description": "A crafty ronin comes to a town divided by two criminal gangs and decides to play them against each other to free the town."
  },
  "One Hundred Fourteen": {
    "title": "Batman Begins",
    "genres": "Action,Adventure",
    "directors": "Christopher Nolan",
    "cast": " Christian Bale as Bruce Wayne / Batman, Michael Caine as Alfred, Liam Neeson as Ducard, Katie Holmes as Rachel Dawes, Gary Oldman as Jim Gordon ",
    "rating": "8.3",
    "description": "After training with his mentor, Batman begins his fight to free crime-ridden Gotham City from the corruption that Scarecrow and the League of Shadows have cast upon it."
  },
  "One Hundred Fifteen": {
    "title": "Up",
    "genres": "Animation,Adventure,Comedy,Family",
    "directors": "Pete Docter,Bob Peterson",
    "cast": " Edward Asner as Carl Fredricksen, Christopher Plummer as Charles Muntz, Jordan Nagai as Russell, Bob Peterson as Dug / Alpha, Delroy Lindo as Beta ",
    "rating": "8.3",
    "description": "Seventy-eight year old Carl Fredricksen travels to Paradise Falls in his home equipped with balloons, inadvertently taking a young stowaway."
  },
  "One Hundred Sixteen": {
    "title": "Some Like It Hot",
    "genres": "Comedy,Romance",
    "directors": "Billy Wilder",
    "cast": " Marilyn Monroe as Sugar Kane Kowalczyk, Tony Curtis as Joe / Josephine / Shell Oil Junior, Jack Lemmon as Jerry / Daphne, George Raft as Spats Colombo, Pat O'Brien as Detective Mulligan ",
    "rating": "8.3",
    "description": "When two male musicians witness a mob hit, they flee the state in an all-female band disguised as women, but further complications set in."
  },
  "One Hundred Seventeen": {
    "title": "The Treasure of the Sierra Madre",
    "genres": "Adventure,Drama,Western",
    "directors": "John Huston",
    "cast": " Humphrey Bogart as Dobbs, Walter Huston as Howard, Tim Holt as Curtin, Bruce Bennett as Cody, Barton MacLane as McCormick ",
    "rating": "8.3",
    "description": "Fred Dobbs and Bob Curtin, two Americans searching for work in Mexico, convince an old prospector to help them mine for gold in the Sierra Madre Mountains."
  },
  "One Hundred Eighteen": {
    "title": "Unforgiven",
    "genres": "Drama,Western",
    "directors": "Clint Eastwood",
    "cast": " Clint Eastwood as Bill Munny, Gene Hackman as Little Bill Daggett, Morgan Freeman as Ned Logan, Richard Harris as English Bob, Jaimz Woolvett as The 'Schofield Kid' ",
    "rating": "8.2",
    "description": "Retired Old West gunslinger William Munny reluctantly takes on one last job, with the help of his old partner and a young man."
  },
  "One Hundred Nineteen": {
    "title": "Der Untergang",
    "genres": "Biography,Drama,History,War",
    "directors": "Oliver Hirschbiegel",
    "cast": " Bruno Ganz as Adolf Hitler, Alexandra Maria Lara as Traudl Junge, Corinna Harfouch as Magda Goebbels, Ulrich Matthes as Joseph Goebbels, Juliane Köhler as Eva Braun ",
    "rating": "8.3",
    "description": "Traudl Junge, the final secretary for Adolf Hitler, tells of the Nazi dictator's final days in his Berlin bunker at the end of WWII."
  },
  "One Hundred Twenty": {
    "title": "Raging Bull",
    "genres": "Biography,Drama,Sport",
    "directors": "Martin Scorsese",
    "cast": " Robert De Niro as Jake La Motta, Cathy Moriarty as Vickie La Motta, Joe Pesci as Joey, Frank Vincent as Salvy, Nicholas Colasanto as Tommy Como ",
    "rating": "8.2",
    "description": "An emotionally self-destructive boxer's journey through life, as the violence and temper that leads him to the top in the ring destroys his life outside it."
  },
  "One Hundred Twenty One": {
    "title": "Die Hard",
    "genres": "Action,Thriller",
    "directors": "John McTiernan",
    "cast": " Bruce Willis as John McClane, Bonnie Bedelia as Holly Gennaro McClane, Reginald VelJohnson as Sgt. Al Powell, Paul Gleason as Deputy Police Chief Dwayne T. Robinson, William Atherton as Richard Thornburg ",
    "rating": "8.2",
    "description": "John McClane, officer of the NYPD, tries to save his wife Holly Gennaro and several others that were taken hostage by German terrorist Hans Gruber during a Christmas party at the Nakatomi Plaza in Los Angeles."
  },
  "One Hundred Twenty Two": {
    "title": "Bacheha-Ye aseman",
    "genres": "Drama,Family",
    "directors": "Majid Majidi",
    "cast": " Mohammad Amir Naji as Ali's Father, Amir Farrokh Hashemian as Ali, Bahare Seddiqi as Zahra, Nafise Jafar-Mohammadi as Roya, Fereshte Sarabandi as Ali's Mother ",
    "rating": "8.4",
    "description": "After a boy loses his sister's pair of shoes, he goes on a series of adventures in order to find them. When he can't, he tries a new way to win a new pair."
  },
  "One Hundred Twenty Three": {
    "title": "The Third Man",
    "genres": "Film-Noir,Mystery,Thriller",
    "directors": "Carol Reed",
    "cast": " Joseph Cotten as Holly Martins, Alida Valli as Anna Schmidt, Orson Welles as Harry Lime, Trevor Howard as Maj. Calloway, Bernard Lee as Sgt. Paine ",
    "rating": "8.3",
    "description": "Pulp novelist Holly Martins travels to shadowy, postwar Vienna, only to find himself investigating the mysterious death of an old friend, Harry Lime."
  },
  "One Hundred Twenty Four": {
    "title": "Heat",
    "genres": "Action,Crime,Drama,Thriller",
    "directors": "Michael Mann",
    "cast": " Al Pacino as Lt. Vincent Hanna, Robert De Niro as Neil McCauley, Val Kilmer as Chris Shiherlis, Jon Voight as Nate, Tom Sizemore as Michael Cheritto ",
    "rating": "8.2",
    "description": "A group of professional bank robbers start to feel the heat from police when they unknowingly leave a clue at their latest heist, while both sides attempt to find balance between their personal lives with their professional lives."
  },
  "One Hundred Twenty Five": {
    "title": "The Great Escape",
    "genres": "Adventure,Drama,History,Thriller,War",
    "directors": "John Sturges",
    "cast": " Steve McQueen as Hilts 'The Cooler King', James Garner as Hendley 'The Scrounger', Richard Attenborough as Bartlett 'Big X', James Donald as Ramsey 'The SBO', Charles Bronson as Danny 'Tunnel King' ",
    "rating": "8.2",
    "description": "Allied prisoners of war plan for several hundred of their number to escape from a German camp during World War II."
  },
  "One Hundred Twenty Six": {
    "title": "Chinatown",
    "genres": "Drama,Mystery,Thriller",
    "directors": "Roman Polanski",
    "cast": " Jack Nicholson as J.J. Gittes, Faye Dunaway as Evelyn Mulwray, John Huston as Noah Cross, Perry Lopez as Escobar, John Hillerman as Yelburton ",
    "rating": "8.2",
    "description": "A private detective hired to expose an adulterer finds himself caught up in a web of deceit, corruption, and murder."
  },
  "One Hundred Twenty Seven": {
    "title": "Ikiru",
    "genres": "Drama",
    "directors": "Akira Kurosawa",
    "cast": " Takashi Shimura as Kanji Watanabe, Shin'ichi Himori as Kimura, Haruo Tanaka as Sakai, Minoru Chiaki as Noguchi, Miki Odagiri as employee Toyo Odagiri ",
    "rating": "8.3",
    "description": "A bureaucrat tries to find a meaning in his life after he discovers he has terminal cancer."
  },
  "One Hundred Twenty Eight": {
    "title": "El laberinto del fauno",
    "genres": "Drama,Fantasy,War",
    "directors": "Guillermo del Toro",
    "cast": " Ivana Baquero as Ofelia, Sergi López as Vidal, Maribel Verdú as Mercedes, Doug Jones as Fauno / Pale Man, Ariadna Gil as Carmen ",
    "rating": "8.2",
    "description": "In the falangist Spain of 1944, the bookish young stepdaughter of a sadistic army officer escapes into an eerie but captivating fantasy world."
  },
  "One Hundred Twenty Nine": {
    "title": "La La Land",
    "genres": "Comedy,Drama,Music,Musical,Romance",
    "directors": "Damien Chazelle",
    "cast": " Ryan Gosling as Sebastian, Emma Stone as Mia, Amiée Conn as Famous Actress, Terry Walters as Linda, Thom Shelton as Coffee Spiller ",
    "rating": "8.2",
    "description": "A jazz pianist falls for an aspiring actress in Los Angeles."
  },
  "One Hundred Thirty": {
    "title": "Tonari no Totoro",
    "genres": "Animation,Family,Fantasy",
    "directors": "Hayao Miyazaki",
    "cast": " Noriko Hidaka as Satsuki, Chika Sakamoto as Mei, Shigesato Itoi as Tatsuo Kusakabe, Sumi Shimamoto as Yasuko Kusakabe, Tanie Kitabayashi as Granny ",
    "rating": "8.2",
    "description": "When two girls move to the country to be near their ailing mother, they have adventures with the wondrous forest spirits who live nearby."
  },
  "One Hundred Thirty One": {
    "title": "Inside Out",
    "genres": "Animation,Adventure,Comedy,Drama,Family,Fantasy",
    "directors": "Pete Docter,Ronnie Del Carmen",
    "cast": " Amy Poehler as Joy, Phyllis Smith as Sadness, Richard Kind as Bing Bong, Bill Hader as Fear, Lewis Black as Anger ",
    "rating": "8.2",
    "description": "After young Riley is uprooted from her Midwest life and moved to San Francisco, her emotions - Joy, Fear, Anger, Disgust and Sadness - conflict on how best to navigate a new city, house, and school."
  },
  "One Hundred Thirty Two": {
    "title": "Logan",
    "genres": "Action,Drama,Sci-Fi,Thriller",
    "directors": "James Mangold",
    "cast": " Hugh Jackman as Logan, Patrick Stewart as Charles, Dafne Keen as Laura, Boyd Holbrook as Pierce, Stephen Merchant as Caliban ",
    "rating": "8.3",
    "description": "In the near future, a weary Logan cares for an ailing Professor X somewhere on the Mexican border. However, Logan's attempts to hide from the world and his legacy are upended when a young mutant arrives, pursued by dark forces."
  },
  "One Hundred Thirty Three": {
    "title": "Ran",
    "genres": "Action,Drama",
    "directors": "Akira Kurosawa",
    "cast": " Tatsuya Nakadai as Lord Hidetora Ichimonji, Akira Terao as Taro Takatora Ichimonji, Jinpachi Nezu as Jiro Masatora Ichimonji, Daisuke Ryû as Saburo Naotora Ichimonji, Mieko Harada as Lady Kaede ",
    "rating": "8.2",
    "description": "In Medieval Japan, an elderly warlord retires, handing over his empire to his three sons. However, he vastly underestimates how the new-found power will corrupt them and cause them to turn on each other...and him."
  },
  "One Hundred Thirty Four": {
    "title": "The Gold Rush",
    "genres": "Adventure,Comedy,Drama,Family",
    "directors": "Charles Chaplin",
    "cast": " Charles Chaplin as The Lone Prospector, Mack Swain as Big Jim McKay, Tom Murray as Black Larsen, Henry Bergman as Hank Curtis, Malcolm Waite as Jack Cameron ",
    "rating": "8.2",
    "description": "A prospector goes to the Klondike in search of gold and finds it and more."
  },
  "One Hundred Thirty Five": {
    "title": "On the Waterfront",
    "genres": "Crime,Drama,Thriller",
    "directors": "Elia Kazan",
    "cast": " Marlon Brando as Terry Malloy, Karl Malden as Father Barry, Lee J. Cobb as Johnny Friendly, Rod Steiger as Charley Malloy, Pat Henning as Kayo Dugan ",
    "rating": "8.2",
    "description": "An ex-prize fighter turned longshoreman struggles to stand up to his corrupt union bosses."
  },
  "One Hundred Thirty Six": {
    "title": "El secreto de sus ojos",
    "genres": "Drama,Mystery,Romance,Thriller",
    "directors": "Juan José Campanella",
    "cast": " Soledad Villamil as Irene Menéndez Hastings, Ricardo Darín as Benjamín Esposito, Carla Quevedo as Liliana Coloto, Pablo Rago as Ricardo Morales, Javier Godino as Isidoro Gómez ",
    "rating": "8.2",
    "description": "A retired legal counselor writes a novel hoping to find closure for one of his past unresolved homicide cases and for his unreciprocated love with his superior - both of which still haunt him decades later."
  },
  "One Hundred Thirty Seven": {
    "title": "Room",
    "genres": "Drama",
    "directors": "Lenny Abrahamson",
    "cast": " Brie Larson as Ma, Jacob Tremblay as Jack, Sean Bridgers as Old Nick, Wendy Crewson as Talk Show Hostess, Sandy McMaster as Veteran ",
    "rating": "8.2",
    "description": "A young boy is raised within the confines of a small shed."
  },
  "One Hundred Thirty Eight": {
    "title": "The Bridge on the River Kwai",
    "genres": "Adventure,Drama,War",
    "directors": "David Lean",
    "cast": " William Holden as Shears, Alec Guinness as Colonel Nicholson, Jack Hawkins as Major Warden, Sessue Hayakawa as Colonel Saito, James Donald as Major Clipton ",
    "rating": "8.2",
    "description": "After settling his differences with a Japanese PoW camp commander, a British colonel co-operates to oversee his men's construction of a railway bridge for their captors - while oblivious to a plan by the Allies to destroy it."
  },
  "One Hundred Thirty Nine": {
    "title": "Blade Runner",
    "genres": "Sci-Fi,Thriller",
    "directors": "Ridley Scott",
    "cast": " Harrison Ford as Rick Deckard, Rutger Hauer as Roy Batty, Sean Young as Rachael, Edward James Olmos as Gaff, M. Emmet Walsh as Bryant ",
    "rating": "8.2",
    "description": "A blade runner must pursue and try to terminate four replicants who stole a ship in space and have returned to Earth to find their creator."
  },
  "One Hundred Forty": {
    "title": "Hauru no ugoku shiro",
    "genres": "Animation,Adventure,Family,Fantasy",
    "directors": "Hayao Miyazaki",
    "cast": " Chieko Baishô as Sofî, Takuya Kimura as Hauru, Akihiro Miwa as Arechi no Majo, Tatsuya Gashûin as Karushifâ, Ryûnosuke Kamiki as Marukuru ",
    "rating": "8.2",
    "description": "When an unconfident young woman is cursed with an old body by a spiteful witch, her only chance of breaking the spell lies with a self-indulgent yet insecure young wizard and his companions in his legged, walking castle."
  },
  "One Hundred Forty One": {
    "title": "Incendies",
    "genres": "Drama,Mystery,War",
    "directors": "Denis Villeneuve",
    "cast": " Mustafa Kamel as Barbier de la Milice / Officer Milice Chrétienne, Hussein Sami as Nihad, Rémy Girard as Notaire Jean Lebel, Mélissa Désormeaux-Poulin as Jeanne Marwan, Maxim Gaudette as Simon Marwan ",
    "rating": "8.2",
    "description": "Twins journey to the Middle East to discover their family history, and fulfill their mother's last wishes."
  },
  "One Hundred Forty Two": {
    "title": "Judgment at Nuremberg",
    "genres": "Drama,War",
    "directors": "Stanley Kramer",
    "cast": " Spencer Tracy as Chief Judge Dan Haywood, Burt Lancaster as Dr. Ernst Janning, Richard Widmark as Col. Tad Lawson, Marlene Dietrich as Mrs. Bertholt, Maximilian Schell as Hans Rolfe ",
    "rating": "8.3",
    "description": "In 1948, an American court in occupied Germany tries four Nazi judges for war crimes."
  },
  "One Hundred Forty Three": {
    "title": "Det sjunde inseglet",
    "genres": "Drama,Fantasy",
    "directors": "Ingmar Bergman",
    "cast": " Gunnar Björnstrand as squire Jöns, Bengt Ekerot as Death, Nils Poppe as Jof / Joseph, Max von Sydow as Antonius Block, Bibi Andersson as Mia / Mary - Jof's wife ",
    "rating": "8.2",
    "description": "A man seeks answers about life, death, and the existence of God as he plays chess against the Grim Reaper during the Black Plague."
  },
  "One Hundred Forty Four": {
    "title": "Lock, Stock and Two Smoking Barrels",
    "genres": "Comedy,Crime",
    "directors": "Guy Ritchie",
    "cast": " Jason Flemyng as Tom, Dexter Fletcher as Soap, Nick Moran as Eddy, Jason Statham as Bacon, Steven Mackintosh as Winston ",
    "rating": "8.2",
    "description": "A botched card game in London triggers four friends, thugs, weed-growers, hard gangsters, loan sharks and debt collectors to collide with each other in a series of unexpected events, all for the sake of weed, cash and two antique shotguns."
  },
  "One Hundred Forty Five": {
    "title": "Mr. Smith Goes to Washington",
    "genres": "Comedy,Drama",
    "directors": "Frank Capra",
    "cast": " Jean Arthur as Saunders, James Stewart as Jefferson Smith, Claude Rains as Senator Joseph Paine, Edward Arnold as Jim Taylor, Guy Kibbee as Governor Hopper ",
    "rating": "8.2",
    "description": "A naive man is appointed to fill a vacancy in the United States Senate. His plans promptly collide with political corruption, but he doesn't back down."
  },
  "One Hundred Forty Six": {
    "title": "Casino",
    "genres": "Crime,Drama",
    "directors": "Martin Scorsese",
    "cast": " Robert De Niro as Sam 'Ace' Rothstein, Sharon Stone as Ginger McKenna, Joe Pesci as Nicky Santoro, James Woods as Lester Diamond, Don Rickles as Billy Sherbert ",
    "rating": "8.2",
    "description": "Greed, deception, money, power, and murder occur between two best friends: a mafia underboss and a casino owner, for a trophy wife over a gambling empire."
  },
  "One Hundred Forty Seven": {
    "title": "A Beautiful Mind",
    "genres": "Biography,Drama",
    "directors": "Ron Howard",
    "cast": " Russell Crowe as John Nash, Ed Harris as Parcher, Jennifer Connelly as Alicia Nash, Christopher Plummer as Dr. Rosen, Paul Bettany as Charles ",
    "rating": "8.2",
    "description": "After , a brilliant but asocial mathematician, accepts secret work in cryptography, his life takes a turn for the nightmarish."
  },
  "One Hundred Forty Eight": {
    "title": "Andrey Rublev",
    "genres": "Biography,Drama,History",
    "directors": "Andrei Tarkovsky",
    "cast": " Anatoliy Solonitsyn as Andrey Rublev, Ivan Lapikov as Kirill, Nikolay Grinko as Daniil Chyornyy, Nikolay Sergeev as Feofan Grek, Irina Tarkovskaya as Durochka ",
    "rating": "8.3",
    "description": "The life, times and afflictions of the fifteenth-century Russian iconographer."
  },
  "One Hundred Forty Nine": {
    "title": "The Elephant Man",
    "genres": "Biography,Drama",
    "directors": "David Lynch",
    "cast": " Anthony Hopkins as Frederick Treves, John Hurt as John Merrick, Anne Bancroft as Mrs. Kendal, John Gielgud as Carr Gomm, Wendy Hiller as Mothershead ",
    "rating": "8.2",
    "description": "A Victorian surgeon rescues a heavily disfigured man who is mistreated while scraping a living as a side-show freak. Behind his monstrous facade, there is revealed a person of intelligence and sensitivity."
  },
  "One Hundred Fifty": {
    "title": "Smultronstället",
    "genres": "Drama,Romance",
    "directors": "Ingmar Bergman",
    "cast": " Victor Sjöström as Dr. Isak Borg, Bibi Andersson as Sara, Ingrid Thulin as Marianne Borg, Gunnar Björnstrand as Dr. Evald Borg, Jullan Kindahl as Agda ",
    "rating": "8.2",
    "description": "After living a life marked by coldness, an aging professor is forced to confront the emptiness of his existence."
  },
  "One Hundred Fifty One": {
    "title": "V for Vendetta",
    "genres": "Action,Drama,Thriller",
    "directors": "James McTeigue",
    "cast": " Natalie Portman as Evey, Hugo Weaving as V, Stephen Rea as Finch, Stephen Fry as Deitrich, John Hurt as Adam Sutler ",
    "rating": "8.2",
    "description": "In a future British tyranny, a shadowy freedom fighter, known only by the alias of 'V', plots to overthrow it with the help of a young woman."
  },
  "One Hundred Fifty Two": {
    "title": "The General",
    "genres": "Action,Adventure,Comedy,Drama,War,Western",
    "directors": "Clyde Bruckman,Buster Keaton",
    "cast": " Buster Keaton as Johnnie Gray, Marion Mack as Annabelle Lee, Glen Cavender as Captain Anderson, Jim Farley as General Thatcher, Frederick Vroom as A Southern General ",
    "rating": "8.2",
    "description": "When Union spies steal an engineer's beloved locomotive, he pursues it single-handedly and straight through enemy lines."
  },
  "One Hundred Fifty Three": {
    "title": "The Wolf of Wall Street",
    "genres": "Biography,Comedy,Crime,Drama",
    "directors": "Martin Scorsese",
    "cast": " Leonardo DiCaprio as Jordan Belfort, Jonah Hill as Donnie Azoff, Margot Robbie as Naomi Lapaglia, Matthew McConaughey as Mark Hanna, Kyle Chandler as Agent Patrick Denham ",
    "rating": "8.2",
    "description": "Based on the true story of , from his rise to a wealthy stock-broker living the high life to his fall involving crime, corruption and the federal government."
  },
  "One Hundred Fifty Four": {
    "title": "Warrior",
    "genres": "Action,Drama,Sport",
    "directors": "Gavin O'Connor",
    "cast": " Joel Edgerton as Brendan Conlon, Tom Hardy as Tommy Conlon, Nick Nolte as Paddy Conlon, Jennifer Morrison as Tess Conlon, Frank Grillo as Frank Campana ",
    "rating": "8.2",
    "description": "The youngest son of an alcoholic former boxer returns home, where he's trained by his father for competition in a mixed martial arts tournament - a path that puts the fighter on a collision course with his estranged, older brother."
  },
  "One Hundred Fifty Five": {
    "title": "Eskiya",
    "genres": "Crime,Drama,Thriller",
    "directors": "Yavuz Turgul",
    "cast": " Sener Sen as Baran, Ugur Yücel as Cumali, Sermin Hürmeriç as Keje, Yesim Salkim as Emel, Kamran Usluer as Berfo ",
    "rating": "8.4",
    "description": "The epic adventures of the legendary Baran the Bandit following his release from prison. After serving 35 years..."
  },
  "One Hundred Fifty Six": {
    "title": "Trainspotting",
    "genres": "Drama",
    "directors": "Danny Boyle",
    "cast": " Ewan McGregor as Renton, Ewen Bremner as Spud, Jonny Lee Miller as Sick Boy, Kevin McKidd as Tommy, Robert Carlyle as Begbie ",
    "rating": "8.2",
    "description": "Renton, deeply immersed in the Edinburgh drug scene, tries to clean up and get out, despite the allure of the drugs and influence of friends."
  },
  "One Hundred Fifty Seven": {
    "title": "Sunrise: A Song of Two Humans",
    "genres": "Drama,Romance",
    "directors": "F.W. Murnau",
    "cast": " George O'Brien as The Man, Janet Gaynor as The Wife, Margaret Livingston as The Woman From the City, Bodil Rosing as The Maid, J. Farrell MacDonald as The Photographer ",
    "rating": "8.3",
    "description": "An allegorical tale about a man fighting the good and evil within him. Both sides are made flesh - one a sophisticated woman he is attracted to and the other his wife."
  },
  "One Hundred Fifty Eight": {
    "title": "Dial M for Murder",
    "genres": "Crime,Film-Noir,Thriller",
    "directors": "Alfred Hitchcock",
    "cast": " Ray Milland as Tony Wendice, Grace Kelly as Margot Wendice, Robert Cummings as Mark Halliday, John Williams as Chief Inspector Hubbard, Anthony Dawson as Charles Swann ",
    "rating": "8.2",
    "description": "An ex-tennis pro carries out a plot to murder his wife. When things go wrong, he improvises a brilliant plan B."
  },
  "One Hundred Fifty Nine": {
    "title": "Gran Torino",
    "genres": "Drama",
    "directors": "Clint Eastwood",
    "cast": " Clint Eastwood as Walt Kowalski, Christopher Carley as Father Janovich, Bee Vang as Thao, Ahney Her as Sue, Brian Haley as Mitch Kowalski ",
    "rating": "8.2",
    "description": "Disgruntled Korean War veteran Walt Kowalski sets out to reform his neighbor, a Hmong teenager who tried to steal Kowalski's prized possession: a 1972 Gran Torino."
  },
  "One Hundred Sixty": {
    "title": "Hacksaw Ridge",
    "genres": "Biography,Drama,History,War",
    "directors": "Mel Gibson",
    "cast": " Andrew Garfield as Desmond Doss, Richard Pyros as Teach, Jacob Warner as James Pinnick, Milo Gibson as Lucky Ford, Darcy Bryce as Young Desmond ",
    "rating": "8.2",
    "description": "WWII American Army Medic , who served during the Battle of Okinawa, refuses to kill people, and becomes the first man in American history to receive the Medal of Honor without firing a shot."
  },
  "One Hundred Sixty One": {
    "title": "The Deer Hunter",
    "genres": "Drama,War",
    "directors": "Michael Cimino",
    "cast": " Robert De Niro as Michael, John Cazale as Stan, John Savage as Steven, Christopher Walken as Nick, Meryl Streep as Linda ",
    "rating": "8.2",
    "description": "An in-depth examination of the ways in which the U.S. Vietnam War impacts and disrupts the lives of people in a small industrial town in Pennsylvania."
  },
  "One Hundred Sixty Two": {
    "title": "Gone with the Wind",
    "genres": "Drama,History,Romance,War",
    "directors": "Victor Fleming,George Cukor,Sam Wood",
    "cast": " Thomas Mitchell as Gerald O'Hara, Barbara O'Neil as Ellen - His Wife, Vivien Leigh as Scarlett - Their Daughter, Evelyn Keyes as Suellen - Their Daughter, Ann Rutherford as Carreen - Their Daughter ",
    "rating": "8.2",
    "description": "A manipulative woman and a roguish man conduct a turbulent romance during the American Civil War and Reconstruction periods."
  },
  "One Hundred Sixty Three": {
    "title": "Fargo",
    "genres": "Crime,Drama,Thriller",
    "directors": "Joel Coen,Ethan Coen",
    "cast": " William H. Macy as Jerry Lundegaard, Steve Buscemi as Carl Showalter, Peter Stormare as Gaear Grimsrud, Kristin Rudrüd as Jean Lundegaard, Harve Presnell as Wade Gustafson ",
    "rating": "8.1",
    "description": "Jerry Lundegaard's inept crime falls apart due to his and his henchmen's bungling and the persistent police work of the quite pregnant Marge Gunderson."
  },
  "One Hundred Sixty Four": {
    "title": "The Big Lebowski",
    "genres": "Comedy,Crime",
    "directors": "Joel Coen,Ethan Coen",
    "cast": " Jeff Bridges as The Dude, John Goodman as Walter Sobchak, Julianne Moore as Maude Lebowski, Steve Buscemi as Theodore Donald 'Donny' Kerabatsos, David Huddleston as The Big Lebowski ",
    "rating": "8.2",
    "description": "'The Dude' Lebowski, mistaken for a millionaire Lebowski, seeks restitution for his ruined rug and enlists his bowling buddies to help get it."
  },
  "One Hundred Sixty Five": {
    "title": "The Sixth Sense",
    "genres": "Drama,Mystery,Thriller",
    "directors": "M. Night Shyamalan",
    "cast": " Bruce Willis as Dr. Malcolm Crowe, Haley Joel Osment as Cole Sear, Toni Collette as Lynn Sear, Olivia Williams as Anna Crowe, Trevor Morgan as Tommy Tammisimo ",
    "rating": "8.1",
    "description": "A boy who communicates with spirits that don't know they're dead seeks the help of a disheartened child psychologist."
  },
  "One Hundred Sixty Six": {
    "title": "The Thing",
    "genres": "Horror,Mystery,Sci-Fi",
    "directors": "John Carpenter",
    "cast": " Kurt Russell as R.J. MacReady, Wilford Brimley as Dr. Blair, T.K. Carter as Nauls, David Clennon as Palmer, Keith David as Childs ",
    "rating": "8.2",
    "description": "A research facility in Antarctica comes across an alien force that can become anything it touches with 100% accuracy. The members must now find out who's human and who's not before it's too late."
  },
  "One Hundred Sixty Seven": {
    "title": "Tôkyô monogatari",
    "genres": "Drama",
    "directors": "Yasujirô Ozu",
    "cast": " Chishû Ryû as Shukichi Hirayama, Chieko Higashiyama as Tomi Hirayama, Setsuko Hara as Noriko Hirayama, Haruko Sugimura as Shige Kaneko, Sô Yamamura as Koichi Hirayama ",
    "rating": "8.2",
    "description": "An old couple visit their children and grandchildren in the city; but the children have little time for them."
  },
  "One Hundred Sixty Eight": {
    "title": "Finding Nemo",
    "genres": "Animation,Adventure,Comedy,Family",
    "directors": "Andrew Stanton,Lee Unkrich",
    "cast": " Albert Brooks as Marlin, Ellen DeGeneres as Dory, Alexander Gould as Nemo, Willem Dafoe as Gill, Brad Garrett as Bloat ",
    "rating": "8.1",
    "description": "After his son is captured in the Great Barrier Reef and taken to Sydney, a timid clownfish sets out on a journey to bring him home."
  },
  "One Hundred Sixty Nine": {
    "title": "No Country for Old Men",
    "genres": "Crime,Drama,Thriller",
    "directors": "Ethan Coen,Joel Coen",
    "cast": " Tommy Lee Jones as Ed Tom Bell, Javier Bardem as Anton Chigurh, Josh Brolin as Llewelyn Moss, Woody Harrelson as Carson Wells, Kelly Macdonald as Carla Jean Moss ",
    "rating": "8.1",
    "description": "Violence and mayhem ensue after a hunter stumbles upon a drug deal gone wrong and more than two million dollars in cash near the Rio Grande."
  },
  "One Hundred Seventy": {
    "title": "Rang De Basanti",
    "genres": "Comedy,Drama,History,Romance",
    "directors": "Rakeysh Omprakash Mehra",
    "cast": " Aamir Khan as Daljeet 'DJ' / Chandrashekhar Azad, Siddharth as Karan R. Singhania / Bhagat Singh, Sharman Joshi as Sukhi / Rajguru, Kunal Kapoor as Aslam / Ashfaqullah Khan, Atul Kulkarni as Laxman Pandey / Ramprasad Bismil ",
    "rating": "8.3",
    "description": "This story is about six friends who help an English filmmaker create a documentary about Indian freedom struggle. During filming, this group of friends learn about those before them and the importance of fighting for their rights."
  },
  "One Hundred Seventy One": {
    "title": "La passion de Jeanne d'Arc",
    "genres": "Biography,Drama,History",
    "directors": "Carl Theodor Dreyer",
    "cast": " Maria Falconetti as Jeanne d'Arc, Eugene Silvain as Évêque Pierre Cauchon, André Berley as Jean d'Estivet, Maurice Schutz as Nicolas Loyseleur, Antonin Artaud as Jean Massieu ",
    "rating": "8.3",
    "description": "A chronicle of the trial of Jeanne d'Arc on charges of heresy, and the efforts of her ecclesiastical jurists to force Jeanne to recant her claims of holy visions."
  },
  "One Hundred Seventy Two": {
    "title": "Cool Hand Luke",
    "genres": "Crime,Drama",
    "directors": "Stuart Rosenberg",
    "cast": " Paul Newman as Luke, George Kennedy as Dragline, J.D. Cannon as Society Red, Lou Antonio as Koko, Robert Drivas as Loudmouth Steve ",
    "rating": "8.2",
    "description": "A man refuses to conform to life in a rural prison."
  },
  "One Hundred Seventy Three": {
    "title": "A Wednesday",
    "genres": "Crime,Drama,Mystery,Thriller",
    "directors": "Neeraj Pandey",
    "cast": " Anupam Kher as Prakash Rathod, Naseeruddin Shah as The Common Man, Jimmy Shergill as Arif Khan, Deepal Shaw as Naina Roy, Aamir Bashir as Jai Singh ",
    "rating": "8.3",
    "description": "A retiring police officer reminisces about the most astounding day of his career. About a case that was never filed but continues to haunt him in his memories - the case of a man and a Wednesday."
  },
  "One Hundred Seventy Four": {
    "title": "Rebecca",
    "genres": "Drama,Mystery,Romance,Thriller",
    "directors": "Alfred Hitchcock",
    "cast": " Laurence Olivier as 'Maxim' de Winter, Joan Fontaine as Mrs. de Winter, George Sanders as Jack Favell, Judith Anderson as Mrs. Danvers, Nigel Bruce as Major Giles Lacy ",
    "rating": "8.2",
    "description": "A self-conscious bride is tormented by the memory of her husband's dead first wife."
  },
  "One Hundred Seventy Five": {
    "title": "How to Train Your Dragon",
    "genres": "Animation,Action,Adventure,Comedy,Family,Fantasy",
    "directors": "Dean DeBlois,Chris Sanders",
    "cast": " Jay Baruchel as Hiccup, Gerard Butler as Stoick, Craig Ferguson as Gobber, America Ferrera as Astrid, Jonah Hill as Snotlout ",
    "rating": "8.1",
    "description": "A hapless young Viking who aspires to hunt dragons becomes the unlikely friend of a young dragon himself, and learns there may be more to the creatures than he assumed."
  },
  "One Hundred Seventy Six": {
    "title": "There Will Be Blood",
    "genres": "Drama,History",
    "directors": "Paul Thomas Anderson",
    "cast": " Daniel Day-Lewis as Daniel Plainview, Martin Stringer as Silver Assay Worker, Matthew Braden Stringer as Silver Assay Worker, Jacob Stringer as Silver Assay Worker, Joseph Mussey as Silver Assay Worker ",
    "rating": "8.1",
    "description": "A story of family, religion, hatred, oil and madness, focusing on a turn-of-the-century prospector in the early days of the business."
  },
  "One Hundred Seventy Seven": {
    "title": "Kill Bill: Vol. 1",
    "genres": "Action,Crime,Thriller",
    "directors": "Quentin Tarantino",
    "cast": " Uma Thurman as The Bride, Lucy Liu as O-Ren Ishii, Vivica A. Fox as Vernita Green, Daryl Hannah as Elle Driver, David Carradine as Bill ",
    "rating": "8.1",
    "description": "The Bride wakens from a four-year coma. The child she carried in her womb is gone. Now she must wreak vengeance on the team of assassins who betrayed her - a team she was once part of."
  },
  "One Hundred Seventy Eight": {
    "title": "Idi i smotri",
    "genres": "Drama,War",
    "directors": "Elem Klimov",
    "cast": " Aleksey Kravchenko as Florya Gaishun, Olga Mironova as Glasha, Liubomiras Laucevicius as Kosach, Vladas Bagdonas as , Jüri Lumiste as a nazi fanatic German officer ",
    "rating": "8.3",
    "description": "After finding an old rifle, a young boy joins the Soviet resistance movement against ruthless German forces and experiences the horrors of World War II."
  },
  "One Hundred Seventy Nine": {
    "title": "Mary and Max",
    "genres": "Animation,Comedy,Drama",
    "directors": "Adam Elliot",
    "cast": " Toni Collette as Mary Daisy Dinkle, Philip Seymour Hoffman as Max Jerry Horovitz, Barry Humphries as Narrator, Eric Bana as Damien, Bethany Whitmore as Young Mary Daisy Dinkle ",
    "rating": "8.2",
    "description": "A tale of friendship between two unlikely pen pals: Mary, a lonely, eight-year-old girl living in the suburbs of Melbourne, and Max, a forty-four-year old, severely obese man living in New York."
  },
  "One Hundred Eighty": {
    "title": "Gone Girl",
    "genres": "Crime,Drama,Mystery,Thriller",
    "directors": "David Fincher",
    "cast": " Ben Affleck as Nick Dunne, Rosamund Pike as Amy Dunne, Neil Patrick Harris as Desi Collings, Tyler Perry as Tanner Bolt, Carrie Coon as Margo Dunne ",
    "rating": "8.1",
    "description": "With his wife's disappearance having become the focus of an intense media circus, a man sees the spotlight turned on him when it's suspected that he may not be innocent."
  },
  "One Hundred Eighty One": {
    "title": "Into the Wild",
    "genres": "Adventure,Biography,Drama",
    "directors": "Sean Penn",
    "cast": " Emile Hirsch as Chris McCandless, Marcia Gay Harden as Billie McCandless, William Hurt as Walt McCandless, Jena Malone as Carine McCandless / Additional Narrator, Brian H. Dierker as Rainey ",
    "rating": "8.1",
    "description": "After graduating from Emory University, top student and athlete Christopher McCandless abandons his possessions, gives his entire $24,000 savings account to charity and hitchhikes to Alaska to live in the wilderness. Along the way, Christopher encounters a series of characters that shape his life."
  },
  "One Hundred Eighty Two": {
    "title": "It Happened One Night",
    "genres": "Comedy,Romance",
    "directors": "Frank Capra",
    "cast": " Clark Gable as Peter, Claudette Colbert as Ellie, Walter Connolly as Andrews, Roscoe Karns as Shapeley, Jameson Thomas as Westley ",
    "rating": "8.2",
    "description": "A spoiled heiress running away from her family is helped by a man who is actually a reporter in need of a story."
  },
  "One Hundred Eighty Three": {
    "title": "Life of Brian",
    "genres": "Comedy",
    "directors": "Terry Jones",
    "cast": " Graham Chapman as Wise Man #2 / Brian Cohen / Biggus Dickus, John Cleese as Wise Man #1 / Reg / Jewish Official / First Centurion / Deadly Dirk / Arthur, Terry Gilliam as Man Even Further Forward / Revolutionary / Jailer / Blood & Thunder Prophet / Frank / Audience Member / Crucifee, Eric Idle as Mr. Cheeky / Stan aka Loretta / Harry the Haggler / Culprit Woman / Warris / Intensely Dull Youth / Jailer's Assistant / Otto / Lead Singer Crucifee, Terry Jones as Mandy Cohen / Colin / Simon the Holy Man / Bob Hoskins / Saintly Passer-by / Alarmed Crucifixion Assistant ",
    "rating": "8.1",
    "description": "Brian is born on the original Christmas, in the stable next door. He spends his life being mistaken for a messiah."
  },
  "One Hundred Eighty Four": {
    "title": "Shutter Island",
    "genres": "Mystery,Thriller",
    "directors": "Martin Scorsese",
    "cast": " Leonardo DiCaprio as Teddy Daniels, Mark Ruffalo as Chuck Aule, Ben Kingsley as Dr. Cawley, Max von Sydow as Dr. Naehring, Michelle Williams as Dolores ",
    "rating": "8.1",
    "description": "In 1954, a U.S. marshal investigates the disappearance of a murderess who escaped from a hospital for the criminally insane."
  },
  "One Hundred Eighty Five": {
    "title": "Platoon",
    "genres": "Drama,War",
    "directors": "Oliver Stone",
    "cast": " Tom Berenger as Sgt. Barnes, Keith David as King, Willem Dafoe as Sgt. Elias, Forest Whitaker as Big Harold, Francesco Quinn as Rhah ",
    "rating": "8.1",
    "description": "A young recruit in Vietnam faces a moral crisis when confronted with the horrors of war and the duality of man."
  },
  "One Hundred Eighty Six": {
    "title": "Hotel Rwanda",
    "genres": "Biography,Drama,History,War",
    "directors": "Terry George",
    "cast": " Don Cheadle as Paul Rusesabagina, Xolani Mali as Policeman, Desmond Dube as Dube, Hakeem Kae-Kazim as George Rutaganda, Tony Kgoroge as Gregoire ",
    "rating": "8.1",
    "description": "Paul Rusesabagina was a hotel manager who housed over a thousand Tutsi refugees during their struggle against the Hutu militia in Rwanda."
  },
  "One Hundred Eighty Seven": {
    "title": "Relatos salvajes",
    "genres": "Comedy,Drama,Thriller",
    "directors": "Damián Szifron",
    "cast": " Darío Grandinetti as Salgado, María Marull as Isabel, Mónica Villa as Profesora Leguizamón, Rita Cortese as Cocinera, Julieta Zylberberg as Moza ",
    "rating": "8.1",
    "description": "Six short stories that explore the extremities of human behavior involving people in distress."
  },
  "One Hundred Eighty Eight": {
    "title": "Rush",
    "genres": "Action,Biography,Drama,History,Sport",
    "directors": "Ron Howard",
    "cast": " Chris Hemsworth as James Hunt, Daniel Brühl as Niki Lauda, Olivia Wilde as Suzy Miller, Alexandra Maria Lara as Marlene Lauda, Pierfrancesco Favino as Clay Regazzoni ",
    "rating": "8.1",
    "description": "The merciless 1970s rivalry between Formula One rivals  and ."
  },
  "One Hundred Eighty Nine": {
    "title": "Le salaire de la peur",
    "genres": "Adventure,Drama,Thriller",
    "directors": "Henri-Georges Clouzot",
    "cast": " Yves Montand as Mario, Charles Vanel as M. Jo, Folco Lulli as Luigi, Peter van Eyck as Bimba, Véra Clouzot as Linda ",
    "rating": "8.2",
    "description": "In a decrepit South American village, four men are hired to transport an urgent nitroglycerine shipment without the equipment that would make it safe."
  },
  "One Hundred Ninty": {
    "title": "Network",
    "genres": "Drama",
    "directors": "Sidney Lumet",
    "cast": " Faye Dunaway as Diana Christensen, William Holden as Max Schumacher, Peter Finch as Howard Beale, Robert Duvall as Frank Hackett, Wesley Addy as Nelson Chaney ",
    "rating": "8.1",
    "description": "A television network cynically exploits a deranged former anchor's ravings and revelations about the news media for its own profit."
  },
  "One Hundred Ninty One": {
    "title": "In the Name of the Father",
    "genres": "Biography,Drama",
    "directors": "Jim Sheridan",
    "cast": " Alison Crosbie as Girl in Pub, Daniel Day-Lewis as Gerry Conlon, Philip King as Guildford Soldier, Emma Thompson as Gareth Peirce, Nye Heron as IRA Man 1 ",
    "rating": "8.1",
    "description": "A man's coerced confession to an IRA bombing he did not commit results in the imprisonment of his father as well. An English lawyer fights to free them."
  },
  "One Hundred Ninty Two": {
    "title": "Stand by Me",
    "genres": "Adventure,Drama",
    "directors": "Rob Reiner",
    "cast": " Wil Wheaton as Gordie Lachance, River Phoenix as Chris Chambers, Corey Feldman as Teddy Duchamp, Jerry O'Connell as Vern Tessio, Kiefer Sutherland as Ace Merrill ",
    "rating": "8.1",
    "description": "After the death of a friend, a writer recounts a boyhood journey to find the body of a missing boy."
  },
  "One Hundred Ninty Three": {
    "title": "Les quatre cents coups",
    "genres": "Crime,Drama",
    "directors": "François Truffaut",
    "cast": " Jean-Pierre Léaud as Antoine Doinel, Claire Maurier as Gilberte Doinel - la mère d'Antoine, Albert Rémy as Julien Doinel, Guy Decomble as the French teacher 'Petite Feuille', Georges Flamant as Mr. Bigey ",
    "rating": "8.1",
    "description": "Moving story of a young boy who, left without attention, delves into a life of petty crime."
  },
  "One Hundred Ninty Four": {
    "title": "Spotlight",
    "genres": "Crime,Drama,History",
    "directors": "Tom McCarthy",
    "cast": " Mark Ruffalo as Mike Rezendes, Michael Keaton as Walter 'Robby' Robinson, Rachel McAdams as Sacha Pfeiffer, Liev Schreiber as Marty Baron, John Slattery as Jr. Ben Bradlee ",
    "rating": "8.1",
    "description": "The true story of how the Boston Globe uncovered the massive scandal of child molestation and cover-up within the local Catholic Archdiocese, shaking the entire Catholic Church to its core."
  },
  "One Hundred Ninty Five": {
    "title": "Mad Max: Fury Road",
    "genres": "Action,Adventure,Sci-Fi,Thriller",
    "directors": "George Miller",
    "cast": " Tom Hardy as Max Rockatansky, Charlize Theron as Imperator Furiosa, Nicholas Hoult as Nux, Hugh Keays-Byrne as Immortan Joe, Josh Helman as Slit ",
    "rating": "8.1",
    "description": "A woman rebels against a tyrannical ruler in postapocalyptic Australia in search for her home-land with the help of a group of female prisoners, a psychotic worshipper, and a drifter named Max."
  },
  "One Hundred Ninty Six": {
    "title": "The Grand Budapest Hotel",
    "genres": "Adventure,Comedy,Drama",
    "directors": "Wes Anderson",
    "cast": " Ralph Fiennes as M. Gustave, F. Murray Abraham as Mr. Moustafa, Mathieu Amalric as Serge X., Adrien Brody as Dmitri, Willem Dafoe as Jopling ",
    "rating": "8.1",
    "description": "The adventures of Gustave H, a legendary concierge at a famous hotel from the fictional Republic of Zubrowka between the first and second World Wars, and Zero Moustafa, the lobby boy who becomes his most trusted friend."
  },
  "One Hundred Ninty Seven": {
    "title": "Ben-Hur",
    "genres": "Adventure,Drama,History",
    "directors": "William Wyler",
    "cast": " Charlton Heston as Judah Ben-Hur, Jack Hawkins as Quintus Arrius, Haya Harareet as Esther, Stephen Boyd as Messala, Hugh Griffith as Sheik Ilderim ",
    "rating": "8.1",
    "description": "When a Jewish prince is betrayed and sent into slavery by a Roman friend, he regains his freedom and comes back for revenge."
  },
  "One Hundred Ninty Eight": {
    "title": "12 Years a Slave",
    "genres": "Biography,Drama,History",
    "directors": "Steve McQueen",
    "cast": " Chiwetel Ejiofor as Solomon Northup, Dwight Henry as Uncle Abram, Dickie Gravois as Overseer, Bryan Batt as Judge Turner, Ashley Dyke as Anna ",
    "rating": "8.1",
    "description": "In the antebellum United States, , a free black man from upstate New York, is abducted and sold into slavery."
  },
  "One Hundred Ninty Nine": {
    "title": "Persona",
    "genres": "Drama,Thriller",
    "directors": "Ingmar Bergman",
    "cast": " Bibi Andersson as Alma, Liv Ullmann as Elisabet Vogler, Margaretha Krook as The Doctor, Gunnar Björnstrand as Mr. Vogler, Jörgen Lindström as Elisabet's Son ",
    "rating": "8.1",
    "description": "A nurse is put in charge of a mute actress and finds that their personas are melding together."
  },
  "Two Hundred": {
    "title": "Million Dollar Baby",
    "genres": "Drama,Sport",
    "directors": "Clint Eastwood",
    "cast": " Clint Eastwood as Frankie Dunn, Hilary Swank as Maggie Fitzgerald, Morgan Freeman as Eddie Scrap-Iron Dupris, Jay Baruchel as Danger Barch, Mike Colter as Big Willie Little ",
    "rating": "8.1",
    "description": "A determined woman works with a hardened boxing trainer to become a professional."
  },
  "Two Hundred One": {
    "title": "Butch Cassidy and the Sundance Kid",
    "genres": "Biography,Crime,Drama,Western",
    "directors": "George Roy Hill",
    "cast": " Paul Newman as Butch Cassidy, Robert Redford as The Sundance Kid, Katharine Ross as Etta Place, Strother Martin as Percy Garris, Henry Jones as Bike Salesman ",
    "rating": "8.1",
    "description": "Two Western bank/train robbers flee to Bolivia when the law gets too close."
  },
  "Two Hundred Two": {
    "title": "Munna Bhai M.B.B.S.",
    "genres": "Comedy,Drama,Romance",
    "directors": "Rajkumar Hirani",
    "cast": " Sunil Dutt as Shri Hari Prasad Sharma, Sanjay Dutt as Murli Prasad 'Munna Bhai' Sharma, Arshad Warsi as Circuit, Gracy Singh as Dr. Suman Asthana, Boman Irani as Dr. J. C. Asthana ",
    "rating": "8.2",
    "description": "A gangster sets out to fulfill his father's dream of becoming a doctor."
  },
  "Two Hundred Three": {
    "title": "Jurassic Park",
    "genres": "Adventure,Sci-Fi,Thriller",
    "directors": "Steven Spielberg",
    "cast": " Sam Neill as Grant, Laura Dern as Ellie, Jeff Goldblum as Malcolm, Richard Attenborough as Hammond, Bob Peck as Muldoon ",
    "rating": "8.1",
    "description": "During a preview tour, a theme park suffers a major power breakdown that allows its cloned dinosaur exhibits to run amok."
  },
  "Two Hundred Four": {
    "title": "Amores perros",
    "genres": "Drama,Thriller",
    "directors": "Alejandro González Iñárritu",
    "cast": " Emilio Echevarría as El Chivo, Gael García Bernal as Octavio, Goya Toledo as Valeria, Álvaro Guerrero as Daniel, Vanessa Bauche as Susana ",
    "rating": "8.1",
    "description": "A horrific car accident connects three stories, each involving characters dealing with loss, regret, and life's harsh realities, all in the name of love."
  },
  "Two Hundred Five": {
    "title": "Salinui chueok",
    "genres": "Crime,Drama,Mystery,Thriller",
    "directors": "Bong Joon Ho",
    "cast": " Kang-ho Song as Detective Park Doo-man, Sang-kyung Kim as Detective Seo Tae-yoon, Roe-ha Kim as Detective Cho Yong-koo, Jae-ho Song as Sergeant Shin Dong-chul, Hee-Bong Byun as Sergeant Koo Hee-bong ",
    "rating": "8.1",
    "description": "In a small Korean province in 1986, three detectives struggle with the case of multiple young women being found raped and murdered by an unknown culprit."
  },
  "Two Hundred Six": {
    "title": "The Maltese Falcon",
    "genres": "Film-Noir,Mystery",
    "directors": "John Huston",
    "cast": " Humphrey Bogart as Samuel Spade, Mary Astor as Brigid O'Shaughnessy, Gladys George as Iva Archer, Peter Lorre as Joel Cairo, Barton MacLane as Lt. of Detectives Dundy ",
    "rating": "8.1",
    "description": "A private detective takes on a case that involves him with three eccentric criminals, a gorgeous liar, and their quest for a priceless statuette."
  },
  "Two Hundred Seven": {
    "title": "Stalker",
    "genres": "Drama,Sci-Fi",
    "directors": "Andrei Tarkovsky",
    "cast": " Alisa Freyndlikh as Zhena Stalkera, Aleksandr Kaydanovskiy as Stalker, Anatoliy Solonitsyn as Pisatel, Nikolay Grinko as Professor, Natalya Abramova as doch Stalkera Marta ",
    "rating": "8.1",
    "description": "A guide leads two men through an area known as the Zone to find a room that grants wishes."
  },
  "Two Hundred Eight": {
    "title": "Le notti di Cabiria",
    "genres": "Drama",
    "directors": "Federico Fellini",
    "cast": " Giulietta Masina as Maria 'Cabiria' Ceccarelli, François Périer as Oscar D'Onofrio, Franca Marzi as Wanda, Dorian Gray as Jessy, Aldo Silvani as The wizard ",
    "rating": "8.2",
    "description": "A waifish prostitute wanders the streets of Rome looking for true love but finds only heartbreak."
  },
  "Two Hundred Nine": {
    "title": "The Princess Bride",
    "genres": "Adventure,Family,Fantasy,Romance",
    "directors": "Rob Reiner",
    "cast": " Cary Elwes as Westley, Mandy Patinkin as Inigo Montoya, Chris Sarandon as Prince Humperdinck, Christopher Guest as Count Rugen, Wallace Shawn as Vizzini ",
    "rating": "8.1",
    "description": "While home sick in bed, a young boy's grandfather reads him a story called The Princess Bride."
  },
  "Two Hundred Ten": {
    "title": "The Truman Show",
    "genres": "Comedy,Drama,Sci-Fi",
    "directors": "Peter Weir",
    "cast": " Jim Carrey as Truman Burbank, Laura Linney as Meryl Burbank / Hannah Gill, Noah Emmerich as Marlon, Natascha McElhone as Lauren / Sylvia, Holland Taylor as Truman's Mother ",
    "rating": "8.1",
    "description": "An insurance salesman/adjuster discovers his entire life is actually a television show."
  },
  "Two Hundred Eleven": {
    "title": "Hachi: A Dog's Tale",
    "genres": "Drama,Family",
    "directors": "Lasse Hallström",
    "cast": " Richard Gere as Parker Wilson, Joan Allen as Cate Wilson, Cary-Hiroyuki Tagawa as Ken, Sarah Roemer as Andy, Jason Alexander as Carl ",
    "rating": "8.1",
    "description": "A college professor's bond with the abandoned dog he takes into his home."
  },
  "Two Hundred Twelve": {
    "title": "Kaze no tani no Naushika",
    "genres": "Animation,Adventure,Fantasy,Sci-Fi",
    "directors": "Hayao Miyazaki",
    "cast": " Sumi Shimamoto as Nausicaä, Mahito Tsujimura as Jihl / Muzu, Hisako Kyôda as Oh-Baba, Gorô Naya as Yupa, Ichirô Nagai as Mito ",
    "rating": "8.1",
    "description": "Warrior/pacifist Princess Nausicaä desperately struggles to prevent two warring nations from destroying themselves and their dying planet."
  },
  "Two Hundred Thirteen": {
    "title": "Before Sunrise",
    "genres": "Drama,Romance",
    "directors": "Richard Linklater",
    "cast": " Ethan Hawke as Jesse, Julie Delpy as Céline, Andrea Eckert as Wife on Train, Hanno Pöschl as Husband on Train, Karl Bruckschwaiger as Guy on Bridge ",
    "rating": "8.1",
    "description": "A young man and woman meet on a train in Europe, and wind up spending one evening together in Vienna. Unfortunately, both know that this will probably be their only night together."
  },
  "Two Hundred Fourteen": {
    "title": "Sholay",
    "genres": "Action,Adventure,Comedy,Drama,Musical,Thriller",
    "directors": "Ramesh Sippy",
    "cast": " Dharmendra as Veeru, Sanjeev Kumar as Thakur Baldev Singh, Hema Malini as Basanti, Amitabh Bachchan as Jai, Jaya Bhaduri as Radha ",
    "rating": "8.3",
    "description": "After his family is murdered by a notorious and ruthless bandit, a former police officer enlists the services of two outlaws to capture the bandit."
  },
  "Two Hundred Fifteen": {
    "title": "Star Wars: Episode VII - The Force Awakens",
    "genres": "Action,Adventure,Fantasy,Sci-Fi",
    "directors": "J.J. Abrams",
    "cast": " Harrison Ford as Han Solo, Mark Hamill as Luke Skywalker, Carrie Fisher as Princess Leia, Adam Driver as Kylo Ren, Daisy Ridley as Rey ",
    "rating": "8.1",
    "description": "Three decades after the Empire's defeat, a new threat arises in the militant First Order. Stormtrooper defector Finn and spare parts scavenger Rey are caught up in the Resistance's search for the missing Luke Skywalker."
  },
  "Two Hundred Sixteen": {
    "title": "The Grapes of Wrath",
    "genres": "Drama,History",
    "directors": "John Ford",
    "cast": " Henry Fonda as Tom Joad, Jane Darwell as Ma Joad, John Carradine as Jim Casy, Charley Grapewin as Grandpa, Dorris Bowdon as Rosasharn ",
    "rating": "8.1",
    "description": "A poor Midwest family is forced off their land. They travel to California, suffering the misfortunes of the homeless in the Great Depression."
  },
  "Two Hundred Seventeen": {
    "title": "Harry Potter and the Deathly Hallows: Part 2",
    "genres": "Adventure,Drama,Fantasy,Mystery",
    "directors": "David Yates",
    "cast": " Ralph Fiennes as Lord Voldemort, Michael Gambon as Professor Albus Dumbledore, Alan Rickman as Professor Severus Snape, Daniel Radcliffe as Harry Potter, Rupert Grint as Ron Weasley ",
    "rating": "8.1",
    "description": "Harry, Ron and Hermione search for Voldemort's remaining Horcruxes in their effort to destroy the Dark Lord as the final battle rages on at Hogwarts."
  },
  "Two Hundred Eighteen": {
    "title": "Rocky",
    "genres": "Drama,Sport",
    "directors": "John G. Avildsen",
    "cast": " Sylvester Stallone as Rocky, Talia Shire as Adrian, Burt Young as Paulie, Carl Weathers as Apollo, Burgess Meredith as Mickey ",
    "rating": "8.1",
    "description": "Rocky Balboa, a small-time boxer, gets a supremely rare chance to fight heavy-weight champion Apollo Creed in a bout in which he strives to go the distance for his self-respect."
  },
  "Two Hundred Nineteen": {
    "title": "Touch of Evil",
    "genres": "Crime,Drama,Film-Noir,Thriller",
    "directors": "Orson Welles",
    "cast": " Charlton Heston as Mike Vargas, Janet Leigh as Susan Vargas, Orson Welles as Police Captain Hank Quinlan, Joseph Calleia as Police Sergeant Pete Menzies, Akim Tamiroff as 'Uncle' Joe Grandi ",
    "rating": "8.1",
    "description": "A stark, perverse story of murder, kidnapping, and police corruption in a Mexican border town."
  },
  "Two Hundred Twenty": {
    "title": "Hera Pheri",
    "genres": "Comedy,Crime,Musical,Thriller",
    "directors": "Priyadarshan",
    "cast": " Akshay Kumar as Raju, Paresh Rawal as Baburao Ganpatrao Apte, Sunil Shetty as Ghanshyam, Tabu as Anuradha Shivshankar Panikar, Om Puri as Kharak Singh ",
    "rating": "8.3",
    "description": "Three unemployed men find the answer to all their money problems when they recieve a call from a kidnapper. However, things do not go as planned."
  },
  "Two Hundred Twenty One": {
    "title": "Prisoners",
    "genres": "Crime,Drama,Mystery,Thriller",
    "directors": "Denis Villeneuve",
    "cast": " Hugh Jackman as Keller Dover, Jake Gyllenhaal as Detective Loki, Viola Davis as Nancy Birch, Maria Bello as Grace Dover, Terrence Howard as Franklin Birch ",
    "rating": "8.1",
    "description": "When Keller Dover's daughter and her friend go missing, he takes matters into his own hands as the police pursue multiple leads and the pressure mounts."
  },
  "Two Hundred Twenty Two": {
    "title": "Gandhi",
    "genres": "Biography,Drama,History",
    "directors": "Richard Attenborough",
    "cast": " Ben Kingsley as Mahatma Gandhi, Candice Bergen as Margaret Bourke-White, Edward Fox as General Dyer, John Gielgud as Lord Irwin, Trevor Howard as Judge Broomfield ",
    "rating": "8.1",
    "description": "Gandhi's character is fully explained as a man of nonviolence. Through his patience, he is able to drive the British out of the subcontinent. And the stubborn nature of Jinnah and his commitment towards Pakistan is portrayed."
  },
  "Two Hundred Twenty Three": {
    "title": "Les diaboliques",
    "genres": "Crime,Drama,Horror,Mystery,Thriller",
    "directors": "Henri-Georges Clouzot",
    "cast": " Simone Signoret as Nicole Horner, Véra Clouzot as Christina Delassalle, Paul Meurisse as Michel Delassalle, Charles Vanel as le commissaire Alfred Fichet, Jean Brochard as le concierge Plantiveau ",
    "rating": "8.1",
    "description": "The wife and mistress of a cruel headmaster conspire to kill him; after the murder is committed, his corpse disappears, and strange events begin to plague the two women."
  },
  "Two Hundred Twenty Four": {
    "title": "Annie Hall",
    "genres": "Comedy,Romance",
    "directors": "Woody Allen",
    "cast": " Woody Allen as Alvy Singer, Diane Keaton as Annie Hall, Tony Roberts as Rob, Carol Kane as Allison, Paul Simon as Tony Lacey ",
    "rating": "8.1",
    "description": "Neurotic New York comedian Alvy Singer falls in love with the ditzy Annie Hall."
  },
  "Two Hundred Twenty Five": {
    "title": "Donnie Darko",
    "genres": "Drama,Sci-Fi,Thriller",
    "directors": "Richard Kelly",
    "cast": " Jake Gyllenhaal as Donnie Darko, Holmes Osborne as Eddie Darko, Maggie Gyllenhaal as Elizabeth Darko, Daveigh Chase as Samantha Darko, Mary McDonnell as Rose Darko ",
    "rating": "8.1",
    "description": "A troubled teenager is plagued by visions of a man in a large rabbit suit who manipulates him to commit a series of crimes, after he narrowly escapes a bizarre accident."
  },
  "Two Hundred Twenty Six": {
    "title": "Catch Me If You Can",
    "genres": "Biography,Crime,Drama",
    "directors": "Steven Spielberg",
    "cast": " Leonardo DiCaprio as Frank Abagnale Jr., Tom Hanks as Carl Hanratty, Christopher Walken as Frank Abagnale, Martin Sheen as Roger Strong, Nathalie Baye as Paula Abagnale ",
    "rating": "8.1",
    "description": "The story of 'Frank Abagnale Jr., before his 19th birthday, successfully forged millions of dollars' worth of checks while posing as a Pan Am pilot, a doctor, and legal prosecutor as a seasoned and dedicated FBI agent pursues him."
  },
  "Two Hundred Twenty Seven": {
    "title": "Monsters, Inc.",
    "genres": "Animation,Adventure,Comedy,Family,Fantasy",
    "directors": "Pete Docter,David Silverman,Lee Unkrich",
    "cast": " John Goodman as Sullivan, Billy Crystal as Mike, Mary Gibbs as Boo, Steve Buscemi as Randall, James Coburn as Waternoose ",
    "rating": "8.1",
    "description": "In order to power the city, monsters have to scare children so that they scream. However, the children are toxic to the monsters, and after a child gets through, 2 monsters realize things may not be what they think."
  },
  "Two Hundred Twenty Eight": {
    "title": "The Bourne Ultimatum",
    "genres": "Action,Mystery,Thriller",
    "directors": "Paul Greengrass",
    "cast": " Matt Damon as Jason Bourne, Julia Stiles as Nicky Parsons, David Strathairn as Noah Vosen, Scott Glenn as Ezra Kramer, Paddy Considine as Simon Ross ",
    "rating": "8.1",
    "description": "Jason Bourne dodges a ruthless CIA official and his agents from a new assassination program while searching for the origins of his life as a trained killer."
  },
  "Two Hundred Twenty Nine": {
    "title": "The Terminator",
    "genres": "Action,Sci-Fi",
    "directors": "James Cameron",
    "cast": " Arnold Schwarzenegger as Terminator, Michael Biehn as Kyle Reese, Linda Hamilton as Sarah Connor, Paul Winfield as Lieutenant Ed Traxler, Lance Henriksen as Detective Hal Vukovich ",
    "rating": "8.0",
    "description": "A seemingly indestructible humanoid cyborg is sent from 2029 to 1984 to assassinate a waitress, whose unborn son will lead humanity in a war against the machines, while a soldier from that war is sent to protect her at all costs."
  },
  "Two Hundred Thirty": {
    "title": "8½",
    "genres": "Drama,Fantasy",
    "directors": "Federico Fellini",
    "cast": " Marcello Mastroianni as Guido Anselmi, Claudia Cardinale as Claudia, Anouk Aimée as Luisa Anselmi, Sandra Milo as Carla, Rossella Falk as Rossella ",
    "rating": "8.1",
    "description": "A harried movie director retreats into his memories and fantasies."
  },
  "Two Hundred Thirty One": {
    "title": "The Wizard of Oz",
    "genres": "Adventure,Family,Fantasy,Musical",
    "directors": "Victor Fleming,George Cukor,Mervyn LeRoy,Norman Taurog,King Vidor",
    "cast": " Judy Garland as Dorothy, Frank Morgan as Professor Marvel / The Wizard of Oz / The Gatekeeper / The Carriage Driver / The Guard, Ray Bolger as 'Hunk' / The Scarecrow, Bert Lahr as 'Zeke' / The Cowardly Lion, Jack Haley as 'Hickory' / The Tin Man ",
    "rating": "8.1",
    "description": "Dorothy Gale is swept away from a farm in Kansas to a magical land of Oz in a tornado and embarks on a quest with her new friends to see the Wizard who can help her return home in Kansas and help her friends as well."
  },
  "Two Hundred Thirty Two": {
    "title": "Groundhog Day",
    "genres": "Comedy,Fantasy,Romance",
    "directors": "Harold Ramis",
    "cast": " Bill Murray as Phil, Andie MacDowell as Rita, Chris Elliott as Larry, Stephen Tobolowsky as Ned, Brian Doyle-Murray as Buster ",
    "rating": "8.0",
    "description": "A weatherman finds himself inexplicably living the same day over and over again."
  },
  "Two Hundred Thirty Three": {
    "title": "Barry Lyndon",
    "genres": "Adventure,Drama,History,War",
    "directors": "Stanley Kubrick",
    "cast": " Ryan O'Neal as Barry Lyndon, Marisa Berenson as Lady Honoria Lyndon, Patrick Magee as The Chevalier du Balibari, Hardy Krüger as Capt. Potzdorf, Steven Berkoff as Lord Ludd ",
    "rating": "8.1",
    "description": "An Irish rogue wins the heart of a rich widow and assumes her dead husband's aristocratic position in 18th-century England."
  },
  "Two Hundred Thirty Four": {
    "title": "La haine",
    "genres": "Crime,Drama",
    "directors": "Mathieu Kassovitz",
    "cast": " Vincent Cassel as Vinz, Hubert Koundé as Hubert, Saïd Taghmaoui as Saïd, Abdel Ahmed Ghili as Abdel, Solo as Santo ",
    "rating": "8.1",
    "description": "24 hours in the lives of three young men in the French suburbs the day after a violent riot."
  },
  "Two Hundred Thirty Five": {
    "title": "Twelve Monkeys",
    "genres": "Mystery,Sci-Fi,Thriller",
    "directors": "Terry Gilliam",
    "cast": " Joseph Melito as Young Cole, Bruce Willis as James Cole, Jon Seda as Jose, Michael Chance as Scarface, Vernon Campbell as Tiny ",
    "rating": "8.0",
    "description": "In a future world devastated by disease, a convict is sent back in time to gather information about the man-made virus that wiped out most of the human population on the planet."
  },
  "Two Hundred Thirty Six": {
    "title": "Jaws",
    "genres": "Adventure,Drama,Thriller",
    "directors": "Steven Spielberg",
    "cast": " Roy Scheider as Chief Martin Brody, Robert Shaw as Quint, Richard Dreyfuss as Matt Hooper, Lorraine Gary as Ellen Brody, Murray Hamilton as Mayor Larry Vaughn ",
    "rating": "8.0",
    "description": "A giant great white shark arrives on the shores of a New England beach resort and wreaks havoc with bloody attacks on swimmers, until a local sheriff teams up with a marine biologist and an old seafarer to hunt the monster down."
  },
  "Two Hundred Thirty Seven": {
    "title": "Mou gaan dou",
    "genres": "Crime,Drama,Mystery,Thriller",
    "directors": "Wai-Keung Lau,Alan Mak",
    "cast": " Andy Lau as Inspector Lau Kin Ming, Tony Chiu-Wai Leung as Chen Wing Yan, Anthony Chau-Sang Wong as SP Wong Chi Shing, Eric Tsang as Hon Sam, Kelly Chen as Dr. Lee Sum Yee ",
    "rating": "8.1",
    "description": "A story between a mole in the police department and an undercover cop. Their objectives are the same: to find out who is the mole, and who is the cop."
  },
  "Two Hundred Thirty Eight": {
    "title": "The Best Years of Our Lives",
    "genres": "Drama,Romance,War",
    "directors": "William Wyler",
    "cast": " Myrna Loy as Milly Stephenson, Fredric March as Al Stephenson, Dana Andrews as Fred Derry, Teresa Wright as Peggy Stephenson, Virginia Mayo as Marie Derry ",
    "rating": "8.1",
    "description": "Three World War II veterans return home to small-town America to discover that they and their families have been irreparably changed."
  },
  "Two Hundred Thirty Nine": {
    "title": "Gangs of Wasseypur",
    "genres": "Action,Crime,Drama,Thriller",
    "directors": "Anurag Kashyap",
    "cast": " Manoj Bajpayee as Sardar Khan, Richa Chadha as Nagma Khatoon, Nawazuddin Siddiqui as Faizal Khan, Tigmanshu Dhulia as Ramadhir Singh, Jameel Khan as Asgar Khan ",
    "rating": "8.3",
    "description": "A clash between Sultan and Shahid Khan leads to the expulsion of Khan from Wasseypur, and ignites a deadly blood feud spanning three generations."
  },
  "Two Hundred Forty": {
    "title": "La battaglia di Algeri",
    "genres": "Drama,War",
    "directors": "Gillo Pontecorvo",
    "cast": " Jean Martin as Col. Mathieu, Yacef Saadi as Djafar, Brahim Hadjadj as Ali La Pointe, Samia Kerbash as One of the girls, Tommaso Neri as Captain ",
    "rating": "8.1",
    "description": "In the 1950s, fear and violence escalate as the people of Algiers fight for independence from the French government."
  },
  "Two Hundred Forty One": {
    "title": "The Help",
    "genres": "Drama",
    "directors": "Tate Taylor",
    "cast": " Emma Stone as Skeeter Phelan, Viola Davis as Aibileen Clark, Bryce Dallas Howard as Hilly Holbrook, Octavia Spencer as Minny Jackson, Jessica Chastain as Celia Foote ",
    "rating": "8.1",
    "description": "An aspiring author during the civil rights movement of the 1960s decides to write a book detailing the African American maids point of view on the white families for which they work, and the hardships they go through on a daily basis."
  },
  "Two Hundred Forty Two": {
    "title": "Dog Day Afternoon",
    "genres": "Biography,Crime,Drama,Thriller",
    "directors": "Sidney Lumet",
    "cast": " Penelope Allen as Sylvia, Sully Boyar as Mulvaney, John Cazale as Sal, Beulah Garrick as Margaret, Carol Kane as Jenny ",
    "rating": "8.0",
    "description": "A man robs a bank to pay for his lover's operation; it turns into a hostage situation and a media circus."
  },
  "Two Hundred Forty Three": {
    "title": "Beauty and the Beast",
    "genres": "Animation,Family,Fantasy,Musical,Romance",
    "directors": "Gary Trousdale,Kirk Wise",
    "cast": " Robby Benson as Beast, Jesse Corti as Lefou, Rex Everhart as Maurice, Angela Lansbury as Mrs. Potts, Paige O'Hara as Belle ",
    "rating": "8.0",
    "description": "A young woman whose father has been imprisoned by a terrifying beast offers herself in his place, unaware that her captor is actually a prince, physically altered by a magic spell."
  },
  "Two Hundred Forty Four": {
    "title": "What Ever Happened to Baby Jane?",
    "genres": "Drama,Horror,Thriller",
    "directors": "Robert Aldrich",
    "cast": " Bette Davis as Baby Jane Hudson, Joan Crawford as Blanche Hudson, Victor Buono as Edwin Flagg, Wesley Addy as Marty Mc Donald, Julie Allred as in 1917 Baby Jane Hudson ",
    "rating": "8.1",
    "description": "A former child star torments her paraplegic sister in their decaying Hollywood mansion."
  },
  "Two Hundred Forty Five": {
    "title": "Faa yeung nin wa",
    "genres": "Drama,Romance",
    "directors": "Kar-Wai Wong",
    "cast": " Maggie Cheung as Su Li-zhen - Mrs. Chan, Tony Chiu-Wai Leung as Chow Mo-wan, Ping Lam Siu as Ah Ping, Tung Cho 'Joe' Cheung as Man living in Mr. Koo's apartment, Rebecca Pan as Mrs. Suen ",
    "rating": "8.1",
    "description": "Two neighbors, a woman and a man, form a strong bond after both suspect extramarital activities of their spouses. However, they agree to keep their bond platonic so as not to commit similar wrongs."
  },
  "Two Hundred Forty Six": {
    "title": "Pirates of the Caribbean: The Curse of the Black Pearl",
    "genres": "Action,Adventure,Fantasy",
    "directors": "Gore Verbinski",
    "cast": " Johnny Depp as Jack Sparrow, Geoffrey Rush as Barbossa, Orlando Bloom as Will Turner, Keira Knightley as Elizabeth Swann, Jack Davenport as Norrington ",
    "rating": "8.0",
    "description": "Blacksmith Will Turner teams up with eccentric pirate \"Captain\" Jack Sparrow to save his love, the governor's daughter, from Jack's former pirate allies, who are now undead."
  },
  "Two Hundred Forty Seven": {
    "title": "Paris, Texas",
    "genres": "Drama",
    "directors": "Wim Wenders",
    "cast": " Harry Dean Stanton as Travis Henderson, Sam Berry as Gas Station Attendant, Bernhard Wicki as Doctor Ulmer, Dean Stockwell as Walt Henderson, Aurore Clément as Anne Henderson ",
    "rating": "8.1",
    "description": "Travis Henderson, an aimless drifter who has been missing for four years, wanders out of the desert and must reconnect with society, himself, his life, and his family."
  },
  "Two Hundred Forty Eight": {
    "title": "PK",
    "genres": "Comedy,Drama,Fantasy,Romance,Sci-Fi",
    "directors": "Rajkumar Hirani",
    "cast": " Aamir Khan as P.K., Anushka Sharma as Jagat Janani / Jaggu, Sanjay Dutt as Bhairon Singh, Boman Irani as Cherry Bajwa, Saurabh Shukla as Tapasvi Maharaj ",
    "rating": "8.2",
    "description": "A stranger in the city asks questions no one has asked before. His childlike curiosity will take him on a journey of love, laughter, and letting go."
  },
  "Two Hundred Forty Nine": {
    "title": "Drishyam",
    "genres": "Crime,Drama,Mystery,Thriller",
    "directors": "Nishikant Kamat",
    "cast": " Ajay Devgn as Vijay Salgaonkar, Shriya Saran as Nandini Salgaonkar, Tabu as Meera Deshmukh, Rajat Kapoor as Mahesh Deshmukh, Ishita Dutta as Anju Salgaonkar ",
    "rating": "8.4",
    "description": "Desperate measures are taken by a man who tries to save his family from the dark side of the law, after they commit an unexpected crime."
  },
  "Two Hundred Fifty": {
    "title": "Swades: We, the People",
    "genres": "Drama",
    "directors": "Ashutosh Gowariker",
    "cast": " Shah Rukh Khan as Mohan Bhargava, Gayatri Joshi as Gita, Kishori Balal as Kaveri amma, Smit Sheth as Chiku, Lekh Tandon as Dadaji - Freedom Fighter ",
    "rating": "8.3",
    "description": "A successful Indian scientist returns to an Indian village to take his nanny to America with him and in the process rediscovers his roots."
  }
}

// Route the incoming request based on type (LaunchRequest, IntentRequest,
// etc.) The JSON body of the request is provided in the event parameter.
exports.handler = function (event, context) {
    try {
        console.log("event.session.application.applicationId=" + event.session.application.applicationId);

        /**
         * Uncomment this if statement and populate with your skill's application ID to
         * prevent someone else from configuring a skill that sends requests to this function.
         */

    if (event.session.application.applicationId !== "amzn-id") {
        context.fail("Invalid Application ID");
     }

        if (event.session.new) {
            onSessionStarted({requestId: event.request.requestId}, event.session);
        }

        if (event.request.type === "LaunchRequest") {
            onLaunch(event.request,
                event.session,
                function callback(sessionAttributes, speechletResponse) {
                    context.succeed(buildResponse(sessionAttributes, speechletResponse));
                });
        } else if (event.request.type === "IntentRequest") {
            onIntent(event.request,
                event.session,
                function callback(sessionAttributes, speechletResponse) {
                    context.succeed(buildResponse(sessionAttributes, speechletResponse));
                });
        } else if (event.request.type === "SessionEndedRequest") {
            onSessionEnded(event.request, event.session);
            context.succeed();
        }
    } catch (e) {
        context.fail("Exception: " + e);
    }
};

/**
 * Called when the session starts.
 */
function onSessionStarted(sessionStartedRequest, session) {
    // add any session init logic here
}

/**
 * Called when the user invokes the skill without specifying what they want.
 */
function onLaunch(launchRequest, session, callback) {
    getWelcomeResponse(callback)
}

/**
 * Called when the user specifies an intent for this skill.
 */
function onIntent(intentRequest, session, callback) {

    var intent = intentRequest.intent
    var intentName = intentRequest.intent.name;

    // dispatch custom intents to handlers here
    if (intentName == "TopIMDBMoviesIntent") {
        handleMovieResponse(intent, session, callback)
    } else if (intentName == "AMAZON.YesIntent") {
        handleYesResponse(intent, session, callback)
    } else if (intentName == "AMAZON.NoIntent") {
        handleNoResponse(intent, session, callback)
    } else if (intentName == "AMAZON.HelpIntent") {
        handleGetHelpRequest(intent, session, callback)
    } else if (intentName == "AMAZON.StopIntent") {
        handleFinishSessionRequest(intent, session, callback)
    } else if (intentName == "AMAZON.CancelIntent") {
        handleFinishSessionRequest(intent, session, callback)
    } else {
        throw "Invalid intent"
    }
}

/**
 * Called when the user ends the session.
 * Is not called when the skill returns shouldEndSession=true.
 */
function onSessionEnded(sessionEndedRequest, session) {

}

// ------- Skill specific logic -------

function getWelcomeResponse(callback) {
    var speechOutput = "Welcome to Top IMDB Movies! I can tell you about top Two Hundred Fifty Movies" + 
    "I can only give detail about one movie at a time. Which movie are you interested in? Start by asking the rank of Movie."

    var reprompt = "Which movie are you interested in? Ask any number from One to Two Hundred Fifty"

    var header = "Top IMDB Movies!"

    var shouldEndSession = false

    var sessionAttributes = {
        "speechOutput" : speechOutput,
        "repromptText" : reprompt
    }

    callback(sessionAttributes, buildSpeechletResponse(header, speechOutput, reprompt, shouldEndSession))

}

function handleMovieResponse(intent, session, callback) {
    var movie = intent.slots.Movie.value

    if (!movies[movie]) {
        var speechOutput = "Please ask a number in range from Zero to Two Hundred Fifty"
        var repromptText = "Try asking another number between Zero and Two Hundred Fifty"
        var header = "Only top Two Hundred Fifty Movies Available"
    } else {
        var title = movies[movie].title
        var genres = movies[movie].genres
        var directors = movies[movie].directors
        var cast = movies[movie].cast
        var rating = movies[movie].rating
        var description = movies[movie].description

        var speechOutput = "The title of the movie is "+title+"It's genre is "+genres+"and directed by "+directors+"featuring "+
                            cast+"having a rating "+rating+"Outline of the movie is as follows "+description+
                            ". Do you want information about other movies"

        var repromptText = "Do you want information about more movies"

        var header = movie
        // var personality_trait = reindeers[reindeer].personality_trait
        // var skill = reindeers[reindeer].skill
        // var speechOutput = capitalizeFirst(reindeer) + " " + personality_trait + " and " + skill + ". Do you want to hear about more reindeer?"    
        // var repromptText = "Do you want to hear about more reindeer?"
        // var header = capitalizeFirst(reindeer)
    }

    var shouldEndSession = false

    callback(session.attributes, buildSpeechletResponse(header, speechOutput, repromptText, shouldEndSession))
}

function handleYesResponse(intent, session, callback) {
    var speechOutput = "Great! Which movie number do you want to find out?"
    var repromptText = speechOutput
    var shouldEndSession = false
    callback(session.attributes, buildSpeechletResponseWithoutCard(speechOutput, repromptText, shouldEndSession))
}

function handleNoResponse(intent, session, callback) {
    handleFinishSessionRequest(intent, session, callback)
}

function handleGetHelpRequest(intent, session, callback) {
    // Ensure that session.attributes has been initialized
    if (!session.attributes) {
        session.attributes = {};
    }

    var speechOutput = "I know about Two Hundred Fifty Movies. I can tell you information about a movie based on the rank of the movie in the IMDB Database" 

    var repromptText = speechOutput

    var shouldEndSession = false

    callback(session.attributes, buildSpeechletResponseWithoutCard(speechOutput, repromptText, shouldEndSession))

}

function handleFinishSessionRequest(intent, session, callback) {
    // End the session with a "Good bye!" if the user wants to quit the game
    callback(session.attributes,
        buildSpeechletResponseWithoutCard("Good bye! Thank you for using Top IMDB Movies!", "", true));
}


// ------- Helper functions to build responses for Alexa -------


function buildSpeechletResponse(title, output, repromptText, shouldEndSession) {
    return {
        outputSpeech: {
            type: "PlainText",
            text: output
        },
        card: {
            type: "Simple",
            title: title,
            content: output
        },
        reprompt: {
            outputSpeech: {
                type: "PlainText",
                text: repromptText
            }
        },
        shouldEndSession: shouldEndSession
    };
}

function buildSpeechletResponseWithoutCard(output, repromptText, shouldEndSession) {
    return {
        outputSpeech: {
            type: "PlainText",
            text: output
        },
        reprompt: {
            outputSpeech: {
                type: "PlainText",
                text: repromptText
            }
        },
        shouldEndSession: shouldEndSession
    };
}

function buildResponse(sessionAttributes, speechletResponse) {
    return {
        version: "1.0",
        sessionAttributes: sessionAttributes,
        response: speechletResponse
    };
}

function capitalizeFirst(s) {
    return s.charAt(0).toUpperCase() + s.slice(1)
}